...
PIO_ENV="${OPEN_DESK_ROM_PIO_ENV:-deskbot_v2}"
...
环境变量（可选）:
  ...
  OPEN_DESK_ROM_PIO_ENV=seeed_xiao_esp32s3 ./flash_rom.sh build

连接方式:
  固件仅使用 USB CDC；烧录完成后保持 USB 连接并启动 PC 服务。
  所有设备共用同一固件，设备码由芯片 eFuse 自动生成。
  详见 README_zh.md
EOF
}
...
resolve_pio() {
  ...
  if [[ -x "$REPO_ROOT/service/.venv/bin/pio" ]]; then
    PIO="$REPO_ROOT/service/.venv/bin/pio"
  elif [[ -x "$REPO_ROOT/service/.venv/Scripts/pio.exe" ]]; then
    PIO="$REPO_ROOT/service/.venv/Scripts/pio.exe"
