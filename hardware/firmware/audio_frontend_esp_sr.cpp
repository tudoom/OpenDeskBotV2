...
bool s_feed_stack_in_psram = false;
bool s_fetch_stack_in_psram = false;
...
SignalStats signal_stats(
  ...
  /* rms is only consumed by the fetch path (VAD volume dBFS).  The feed and
   * output-health paths run per AFE chunk and would otherwise pay a double
   * sqrt each frame for a value nobody reads; square_sum stays available
   * for the 5 s diagnostic windows either way. */
  if (need_rms) {
...
void cleanup_failed_setup() {
  ...
  /* 两个 worker 的栈是 deskbot_task_create_pinned 的静态 PSRAM 栈，
   * vTaskDelete 不会回收它们。本函数只在开机 AFE 建立失败时跑一次，之后
   * 走 raw-mic 回退、同一次启动内不再重建，接受一次性的 PSRAM 留存。 */
  if (s_feed_task) {
...
bool audio_frontend_setup() {
  ...
  /* feed/fetch 栈放 PSRAM（deskbot_task_create_pinned，失败回落内部堆）：
   * 两个 worker 只跑 AFE feed/fetch、队列和日志，模型从 flash mmap 只读，
   * 从不写 flash/NVS，满足 PSRAM 栈前提；合计省 16KB 内部 RAM。 */
  TaskHandle_t feed_created = nullptr;
  const BaseType_t feed_task_rc = deskbot_task_create_pinned(
  ...
  const BaseType_t fetch_task_rc = deskbot_task_create_pinned(
  ...
  log_info("[ESP-SR] worker stacks feed=%s fetch=%s",
           s_feed_stack_in_psram ? "psram" : "internal",
           s_fetch_stack_in_psram ? "psram" : "internal");
  s_output_healthy.store(true, std::memory_order_release);
  s_identical_processed_frames = 0;
  s_processed_recovery_frames = 0;
