...
std::atomic<uint32_t> s_keep_warm_misses{0};
std::atomic<uint32_t> s_last_fb_get_ms{0};
std::atomic<uint32_t> s_uploads{0};
unsigned long s_last_stat_log_ms = 0;
...
/* 连续取帧失败多少次才重建驱动。实测单次 no_frame 抖动很常见，3 次就拆驱动
 * 会把一次抖动放大成「拆了装不回来」的永久失明（见下方内存门槛）。 */
constexpr uint32_t kCameraFailureRecoveryThreshold = 8u;
constexpr uint32_t kCameraRecoveryMinIntervalMs = 30000u;
/* 重建驱动需要一块连续内部 RAM（DMA 描述符与驱动结构）。开机时最大连续块
 * ~128KB 时初始化正常；跑久后 WiFi + RTC 解码 + AFE 把内部 RAM 吃到最大块
 * ~31KB，此时 esp_camera_init 表面成功但传感器永远出不来帧，validation 失败
 * 再 deinit，循环到死。低于门槛时：驱动还活着就绝不拆，驱动已拆就不再徒劳
 * 重建，等内存回来（USB 在线时 WiFi 已停，通常能回来）。 */
constexpr uint32_t kCameraReinitMinLargestBlockBytes = 40u * 1024u;
/* 驱动已拆时允许尝试初始化的门槛要低得多：实测这台机器在最大连续块 31.7KB
 * 时 esp_camera_init 能成功（问题出在拆装后传感器不出帧，不是分配失败），
 * 40KB 会把开机初始化失败后的正常重建也一并挡死，相机永远起不来。 */
constexpr uint32_t kCameraSetupMinLargestBlockBytes = 24u * 1024u;
constexpr uint32_t kCameraLowMemorySetupRetryMs = 60000u;
constexpr uint32_t kCameraSetupThrashRetryMs = 120000u;
constexpr uint32_t kCameraSetupThrashAfterFailures = 4u;

static uint32_t camera_internal_largest_block() {
...
/* 空闲保温取帧间隔（见 camera_uplink_task 的 interval==0 分支）。 */
constexpr uint32_t kCameraKeepWarmIntervalMs = 1000u;
...
CameraUploadResult try_upload_one_frame(uint32_t expected_epoch) {
...
#endif
  const uint32_t fb_get_started_ms = millis();
  camera_fb_t* fb = esp_camera_fb_get();
  s_last_fb_get_ms.store(millis() - fb_get_started_ms, std::memory_order_release);
#if DESKBOT_CAMERA_DIAGNOSTICS
  ...
  /*
   * 2026-09-07：PC 端解码器每 2–3 帧就报一次载荷 CRC 错、长度却完全对齐
   * （恰好丢一整帧、不失步）——帧内容在"算完 CRC"到"写完 USB"之间变了：
   * fb_count=1 时驱动 DMA 会往还没归还的帧缓冲继续写。先把 JPEG 拷到私有
   * 缓冲、立刻归还 fb，再从副本发送；CRC 与线上的字节从此是同一份数据。
   */
  uint8_t* private_copy = nullptr;
  if (converted_jpeg == nullptr) {
    private_copy = static_cast<uint8_t*>(
        heap_caps_malloc(jpeg_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (private_copy == nullptr) {
      private_copy = static_cast<uint8_t*>(malloc(jpeg_len));
    }
    if (private_copy != nullptr) {
      memcpy(private_copy, jpeg_payload, jpeg_len);
      jpeg_payload = private_copy;
      esp_camera_fb_return(fb);
      fb = nullptr;
    }
  }
  ...
  if (sent) {
    ...
    s_uploads.fetch_add(1, std::memory_order_relaxed);
  } else {
...
void camera_uplink_task(void*) {
  ...
  unsigned long last_keep_warm_ms = 0;
  for (;;) {
    if (!s_camera_ok.load(std::memory_order_acquire)) {
      ...
      const uint32_t largest_block = camera_internal_largest_block();
      if (largest_block < kCameraSetupMinLargestBlockBytes) {
        /* 内存真的不足以分配驱动：不要每分钟徒劳 init/deinit 一次（每次都会把
         * 传感器和 XCLK 折腾一遍），等内存回来再试。 */
        next_setup_attempt_ms = millis() + kCameraLowMemorySetupRetryMs;
        log_warn(
            "[CAMERA] setup deferred: internal largest_block=%u < %u "
            "retry_in=%ums",
            static_cast<unsigned>(largest_block),
            static_cast<unsigned>(kCameraSetupMinLargestBlockBytes),
            static_cast<unsigned>(kCameraLowMemorySetupRetryMs));
        continue;
      }
      ...
      if (setup_camera()) {
        ...
        /* 连续多次「init 成功但 validation 失败」= 传感器/内存状态没变，
         * 继续每 30s 拆装只会空转，拉长到 2 分钟一次。 */
        const bool thrashing =
            setup_retry_failures >= kCameraSetupThrashAfterFailures;
        const uint32_t retry_ms =
            no_memory
                ? kCameraNoMemoryRetryMs
                : thrashing
                      ? kCameraSetupThrashRetryMs
    ...
    {
      /* 每 10s 一行 WARN（会转发到 PC）：相机任务到底在干什么。2026-09-07
       * 排查"一帧都没有"时，INFO 级日志不转发，PC 侧完全是盲的。 */
      const unsigned long stat_now = millis();
      if (s_last_stat_log_ms == 0 || stat_now - s_last_stat_log_ms >= 10000u) {
        s_last_stat_log_ms = stat_now;
        log_warn("[CAMERA] stat interval=%ums pending=%u last_frame_age=%lums "
                 "last_upload_age=%lums fb_get_last=%ums uploads=%u "
                 "capture_fail=%u transport_fail=%u keep_warm_miss=%u",
                 static_cast<unsigned>(s_interval_ms.load(std::memory_order_acquire)),
                 static_cast<unsigned>(s_pending_snapshots.load(std::memory_order_acquire)),
                 stat_now - s_last_frame_ms.load(std::memory_order_acquire),
                 stat_now - s_last_upload_ms.load(std::memory_order_acquire),
                 static_cast<unsigned>(s_last_fb_get_ms.load(std::memory_order_acquire)),
                 static_cast<unsigned>(s_uploads.load(std::memory_order_acquire)),
                 static_cast<unsigned>(s_capture_failures.load(std::memory_order_acquire)),
                 static_cast<unsigned>(s_transport_failures.load(std::memory_order_acquire)),
                 static_cast<unsigned>(s_keep_warm_misses.load(std::memory_order_acquire)));
      }
    }
#if DESKBOT_CAMERA_DIAGNOSTICS
...
#endif
    if (capture_paused()) {
      /* 之前这里静默等待：相机 ready、取帧 0 失败、却一帧不上传，PC 端毫无线索
       * （2026-09-07 排查一整天）。限频打一条 WARN，把暂停原因交代清楚。 */
      static unsigned long s_last_pause_log_ms = 0;
      const unsigned long pause_now = millis();
      if (s_last_pause_log_ms == 0 || pause_now - s_last_pause_log_ms >= 10000u) {
        s_last_pause_log_ms = pause_now;
        log_warn("[CAMERA] capture paused speaker_audible=%d speaker_busy=%d "
                 "rtc_interruptible=%d pending_snapshots=%u interval=%ums",
                 deskbot_uplink_speaker_audible() ? 1 : 0,
                 audio_play_speaker_busy() ? 1 : 0,
                 deskbot_uplink_rtc_interruptible() ? 1 : 0,
    ...
    if (!snapshot_requested && interval == 0) {
      /* 保温：空闲时每秒取一帧直接丢掉，不上传。OV3660 的 DMA 停掉后
       * 重新起流首帧要 4–6s，one-shot 2.5s 窗口永远等不到，首页轮询就变成
       * 每 10s 一次冷启动风暴（2026-09-07）。取帧不占 USB，只占相机 DMA。 */
      const unsigned long idle_now = millis();
      if (last_keep_warm_ms == 0 ||
          idle_now - last_keep_warm_ms >= kCameraKeepWarmIntervalMs) {
        last_keep_warm_ms = idle_now;
        const uint32_t warm_started_ms = millis();
        camera_fb_t* warm = esp_camera_fb_get();
        s_last_fb_get_ms.store(millis() - warm_started_ms, std::memory_order_release);
        if (warm != nullptr) {
          if (camera_source_frame_valid(warm)) {
            s_last_frame_ms.store(millis(), std::memory_order_release);
          }
          esp_camera_fb_return(warm);
        } else {
          s_keep_warm_misses.fetch_add(1, std::memory_order_relaxed);
        }
      }
      (void)ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(25));
      continue;
    ...
    if (consecutive_camera_failures >=
      ...
      const uint32_t largest_block = camera_internal_largest_block();
      if (largest_block < kCameraReinitMinLargestBlockBytes) {
        /* 驱动还活着但内存已不够重建：拆了就装不回来，宁可留着继续尝试
         * 取帧（帧可能在下一次 USB 空闲时回来）。 */
        log_warn(
            "[CAMERA] driver reset skipped: internal largest_block=%u < %u "
            "failures=%u (keeping driver)",
            static_cast<unsigned>(largest_block),
            static_cast<unsigned>(kCameraReinitMinLargestBlockBytes),
            static_cast<unsigned>(consecutive_camera_failures));
        consecutive_camera_failures = 0;
...
bool setup_camera() {
  ...
  /* OV3660（新批次模组）：PSRAM-DMA 模式下 LCD_CAM 采不到帧（信号线上 PCLK/
   * VSYNC 都在跳，fb_get 却永远超时），保持 legacy 内部 DMA 路径。 */
  bool skip_psram_dma_for_sensor = false;
  if (err == ESP_OK) {
    sensor_t* probe_sensor = esp_camera_sensor_get();
    if (probe_sensor != nullptr && probe_sensor->id.PID == OV3660_PID) {
      skip_psram_dma_for_sensor = true;
      log_warn("[CAMERA] OV3660: keeping legacy DMA mode (psram_dma skipped)");
    }
  }
  if (err == ESP_OK && psram_dma_requested && !skip_psram_dma_for_sensor &&
  ...
  /* 在预热校验之前就把传感器身份打出来：I2C 探测成功但没有帧时，凭 PID 能
   * 区分"换了模组型号"与"排线/信号线没接好"，否则只能看到 no_frame。 */
  log_warn("[CAMERA] sensor detected pid=0x%x ver=0x%x mid=%02x:%02x xclk=%u",
           ...
           static_cast<unsigned>(sensor->xclk_freq_hz));
  if (sensor->id.PID == OV3660_PID) {
    /* 默认 10MHz XCLK 是按 OV2640 调的；OV3660 的 PLL 以 20MHz 为基准，
     * 10MHz 下 I2C 探测正常但一帧也出不来（新批次模组实测）。运行期把
     * XCLK 切到 20MHz 再预热。 */
    if (sensor->set_xclk != nullptr && sensor->xclk_freq_hz < 20000000) {
      const int xclk_rc = sensor->set_xclk(sensor, kCameraXclkTimer, 20);
      log_warn("[CAMERA] OV3660: xclk 10MHz -> 20MHz rc=%d now=%u", xclk_rc,
               static_cast<unsigned>(sensor->xclk_freq_hz));
    }
    ...
    /* 传感器级画质（移植自早期分支的 camera.cpp，OV3660 驱动实现，全部可选）：
     * - sharpness +3（范围 -3..3）：边缘增强，QVGA 上传给视觉模型时文字/
     *   轮廓明显更清；代价是噪点也会被锐化，所以 denoise 保持驱动默认
     *   （开 denoise 会抹掉细节，得不偿失）。
     * - contrast +1：早期分支实测 +2 浅色字略清但高光过曝裁切，+1 折中。
     * - bpc/wpc=1：坏点/白点校正，去掉低照度下的固定亮点。
     * 只调画质寄存器，不改 QVGA / 按需抓拍策略。驱动缺回调就跳过。 */
    if (sensor->set_sharpness != nullptr) {
      (void)apply_sensor_int_setting(
          sensor, "sharpness", sensor->set_sharpness, 3, false);
    }
    if (sensor->set_contrast != nullptr) {
      (void)apply_sensor_int_setting(
          sensor, "contrast", sensor->set_contrast, 1, false);
    }
    if (sensor->set_bpc != nullptr) {
      (void)apply_sensor_int_setting(
          sensor, "bpc", sensor->set_bpc, 1, false);
    }
    if (sensor->set_wpc != nullptr) {
      (void)apply_sensor_int_setting(
          sensor, "wpc", sensor->set_wpc, 1, false);
    }
  }
  {
    /* 120ms 直接采样 VSYNC/HREF/PCLK 电平跳变（传感器已上电、XCLK 已起）：
     * 三根全 0 = 排线/信号线没通；只有 PCLK = 传感器没出流；全有 = 数据到了
     * 但 DMA/帧缓冲那边有问题。预热失败时只有这一行能定位到硬件层。 */
    CameraProbeRecord activity;
    sample_camera_sync_activity(&activity);
    log_warn("[CAMERA] sync activity 120ms edges(v/h/p)=%u/%u/%u samples=%u",
             static_cast<unsigned>(activity.vsync_edges),
             static_cast<unsigned>(activity.href_edges),
             static_cast<unsigned>(activity.pclk_edges),
             static_cast<unsigned>(activity.samples));
  }
