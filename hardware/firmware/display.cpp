...
/*
 * 上一帧 + 当前帧共 12 个 StoredLayer（约 30 KiB）原先是静态内部 DRAM。
 * 接入 WiFi 传输后 net80211/lwip/pp 的静态段挤占了同一块内存，麦克风采集
 * 任务 8 KiB 栈在启动时分配失败，runtime 守护随即进入 mic_task_stall 重启
 * 循环。与 s_voice_base 同一处理：整体搬进 PSRAM（setup_display 里成功分配
 * 后才继续），仅渲染任务串行访问，宏保持所有引用点写法不变。
 */
struct PbLayerStorage {
  StoredLayer prev_bg{};
...
static PbLayerStorage* s_pb_layers = nullptr;
static JPEGDEC* s_jpeg_dec_store = nullptr;

static void pb_layer_storage_init() {
  if (s_pb_layers) {
    return;
  }
  s_pb_layers = static_cast<PbLayerStorage*>(heap_caps_calloc(
      1, sizeof(PbLayerStorage), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!s_pb_layers) {
    s_pb_layers = static_cast<PbLayerStorage*>(
        heap_caps_calloc(1, sizeof(PbLayerStorage), MALLOC_CAP_8BIT));
  }
  if (s_pb_layers) {
    log_info("[DISPLAY] pb layer storage allocated bytes=%u",
             static_cast<unsigned>(sizeof(PbLayerStorage)));
  } else {
    log_error("[DISPLAY] pb layer storage allocation failed bytes=%u",
              static_cast<unsigned>(sizeof(PbLayerStorage)));
  }
  if (!s_jpeg_dec_store) {
    void* jpeg_mem = heap_caps_malloc(
        sizeof(JPEGDEC), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!jpeg_mem) {
      jpeg_mem = heap_caps_malloc(sizeof(JPEGDEC), MALLOC_CAP_8BIT);
    }
    if (jpeg_mem) {
      s_jpeg_dec_store = new (jpeg_mem) JPEGDEC();
      log_info("[DISPLAY] jpeg decoder allocated bytes=%u",
               static_cast<unsigned>(sizeof(JPEGDEC)));
    } else {
      log_error("[DISPLAY] jpeg decoder allocation failed bytes=%u",
                static_cast<unsigned>(sizeof(JPEGDEC)));
    }
  }
}

#define s_prev_bg (s_pb_layers->prev_bg)
#define s_prev_nose (s_pb_layers->prev_nose)
#define s_prev_mouth (s_pb_layers->prev_mouth)
#define s_prev_eye_l (s_pb_layers->prev_eye_l)
#define s_prev_eye_r (s_pb_layers->prev_eye_r)
#define s_prev_extra (s_pb_layers->prev_extra)
#define s_pb_curr_bg (s_pb_layers->curr_bg)
#define s_pb_curr_nose (s_pb_layers->curr_nose)
#define s_pb_curr_mouth (s_pb_layers->curr_mouth)
#define s_pb_curr_eye_l (s_pb_layers->curr_eye_l)
#define s_pb_curr_eye_r (s_pb_layers->curr_eye_r)
#define s_pb_curr_extra (s_pb_layers->curr_extra)
...
/* JPEG 解码器实例约 18 KiB：跟 12 个图层一样搬进 PSRAM，把内部 DRAM 留给
 * WiFi 运行时堆与各任务栈。解码仍在 display_render 任务里串行执行。 */
#define s_jpeg_dec (*s_jpeg_dec_store)
 ...
 *  PC 服务期间的「请先连接PC服务」待机屏。
 *
 *  几何与颜色照搬服务端 deskbot-face.json 系统 `idle`（日常待机）场景第 0 帧
 *  （睁眼保持帧，画布坐标），使未连接时的待机脸与连接后的默认表情完全一致；
 *  服务端那边改了 idle 场景，这里要同步（三端锁步：pb/primitive_spec.py
 *  BUILTIN_FACE_ELEMENTS + tests/test_primitive_spec_lockstep.py）。
 *  c=2047 即 RGB565 0x07FF 青色；鼻子取控制台默认部件色 #ffd23f → 0xFE87。 */
static constexpr uint16_t kBuiltinFaceColor = 0x07FF;
static constexpr uint16_t kBuiltinNoseColor = 0xFE87;
...
static void pb_build_builtin_face_layers() {
  ...
  /* eye_l: ellipse_fill x=105 y=96 rw=11 rh=11；eye_r: x=181 y=97 */
  StoredPrim eye{};
  eye.shape = PbShape::EllipseFill;
  eye.color = kBuiltinFaceColor;
  eye.w = 11;
  eye.h = 11;
  eye.stroke_width = 1;
  eye.x = 105;
  ...
  /* nose: circle x=142 y=124 r=11（黄色填充，与控制台默认鼻子部件一致） */
  StoredPrim nose{};
  nose.shape = PbShape::Circle;
  nose.color = kBuiltinNoseColor;
  nose.x = 142;
  nose.y = 124;
  nose.r = 11;
  nose.rot_cx = nose.x;
  nose.rot_cy = nose.y;
  nose.stroke_width = 1;
  s_pb_curr_nose.prims[s_pb_curr_nose.count++] = nose;

  /* mouth[0]: round_rect x=147 y=153 w=60 h=18 radius=9（青色填充） */
  StoredPrim mouth{};
  mouth.shape = PbShape::RoundRect;
  mouth.color = kBuiltinFaceColor;
  mouth.x = 147;
  mouth.y = 153;
  mouth.w = 60;
  mouth.h = 18;
  ...
  mouth.rot_cx = mouth.x + mouth.w / 2;
  mouth.rot_cy = mouth.y + mouth.h / 2;
  mouth.stroke_width = 1;
  s_pb_curr_mouth.prims[s_pb_curr_mouth.count++] = mouth;

  /* mouth[1]: round_rect x=152 y=159 w=50 h=3 radius=1（黑色嘴缝，叠在上面） */
  StoredPrim slit{};
  slit.shape = PbShape::RoundRect;
  slit.color = DESKBOT_DISPLAY_COLOR_BLACK;
  slit.x = 152;
  slit.y = 159;
  slit.w = 50;
  slit.h = 3;
  slit.r = 1;
  slit.rot_cx = slit.x + slit.w / 2;
  slit.rot_cy = slit.y + slit.h / 2;
  slit.stroke_width = 1;
  s_pb_curr_mouth.prims[s_pb_curr_mouth.count++] = slit;
}
...
/* 待机屏：内建默认脸（= 服务端 idle 场景）+ 底部文案「请先连接PC服务」。
 ...
 * u8g2_font_wqy12_t_gb2312b，见 display_text.cpp），「请先连接服务」六个汉字与
 * "PC" 均已覆盖，无需另嵌位图；若未来文案超出 GB2312 子集，需在
 * DESKBOT_DISPLAY_CJK_FONT 处扩展字库。 */
static constexpr char kStandbyHintText[] = "请先连接PC服务";
/* wqy12：汉字宽 12px、ASCII 约 7px → 文案宽约 6*12+2*7=86px；
...
bool ensure_render_task() {
  ...
  /* 渲染态缓冲（12 图层 + JPEG 解码器）在 PSRAM；没有它们渲染任务一步都
   * 走不了，分配失败时保持运行时未启动，面板走直写降级路径。 */
  pb_layer_storage_init();
  if (!s_pb_layers || !s_jpeg_dec_store) {
    display_log_runtime_allocation_failure("pb_layer_storage");
    return false;
  }
  if (!s_queue) {
  ...
  if (!s_task) {
    ...
    /* 32KB 栈放 PSRAM（deskbot_task_create_pinned，失败回落内部堆）：渲染
     * 任务只写 SPI 屏、解 JPEG、读 flash 里的字体，从不写 flash/NVS。 */
    TaskHandle_t created_task = nullptr;
    s_task_stack_uses_psram = false;
    const BaseType_t rc = deskbot_task_create_pinned(
        display_render_task, "display_render", 32 * 1024, nullptr, 2,
        &created_task, APP_CPU_NUM, &s_task_stack_uses_psram);
