...
/**
 * True while the DBOT session machine is bound to the WiFi TCP byte link
 * instead of USB CDC.  USB always takes the link back when a host talks.
 */
...
/** loopTask 两次 poll 之间的历史最大间隔（停发取证用）。 */
uint32_t usb_transport_poll_max_gap_ms(void);
/* 三方对账取证：最近收到的需确认帧序号 / 最近发出的确认序号。 */
...
/* 片内温度（℃，取整；0 = 读不到）。hello / 心跳 / [MEM] 日志都带它。 */
