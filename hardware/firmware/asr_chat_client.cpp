...
/* 7.6 KiB 的业务层对象放 PSRAM：全局构造期 PSRAM 堆已就绪，成员访问都在
 * 任务上下文（非 ISR），慢一点无感；内部 DRAM 留给 WiFi/lwIP 与任务栈。 */
static AsrChatClient* make_asr_chat_client() {
  void* mem = heap_caps_malloc(sizeof(AsrChatClient),
                               MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (mem == nullptr) {
    mem = heap_caps_malloc(sizeof(AsrChatClient), MALLOC_CAP_8BIT);
  }
  return new (mem) AsrChatClient();
}
AsrChatClient& asrChatClient = *make_asr_chat_client();
...
void AsrChatClient::pbDrainWorkersForNewSequence(bool replace_display,
  ...
  if (pb_expect_bin_) {
    /* 新序列来时上一条还欠着二进制：PC 端的 bundle 锁本应杜绝这种交错。
     * 打 WARN 让日志一眼看出是谁插队（否则只会在几帧后看到
     * "BIN fragment overflow"，很难回溯）。 */
    log_warn("[PB] new sequence while awaiting BIN req=%s received=%u/%u "
             "bin=%u/%u",
             pb_req_.c_str(), (unsigned)pb_expect_bin_received_,
             (unsigned)pb_expect_bin_len_,
             (unsigned)(pb_pending_bin_cursor_ + 1),
             (unsigned)pb_pending_bin_count_);
  }
  if (cancel_terminals) {
...
void AsrChatClient::dispatchUsbFrame(const DeskbotUsbRxFrame& frame) {
  ...
  if (frame.channel == DESKBOT_USB_PB_WIRE) {
    if ((frame.flags & DESKBOT_USB_FLAG_JSON) != 0) {
      ...
      if (usb_pb_json_active_) {
        /* 2026-09-07：PC 端并发的两条位图表情 PB 曾把二进制插进另一条的
         * JSON 分片之间。这里以前直接 resetTransportSession——整条 USB 会话
         * 被拆掉、麦克风/音频全部重来，PC 端表现为 "frame ACK timeout"。
         * 帧本身是完整且 CRC 正确的，只是 PB 层错位：只判当前 PB 序列失败，
         * 传输会话继续活着，下一条完整 PB 立刻能恢复。 */
        log_warn("[USB] binary PB interleaved with JSON fragments "
                 "json_len=%u bin_len=%u req=%s",
                 static_cast<unsigned>(usb_pb_json_len_),
                 static_cast<unsigned>(frame.payload_length),
                 pb_req_.c_str());
        resetUsbPbJsonAssembly();
        pbProtocolError("binary interleaved with JSON fragments");
        return;
      }
...
bool AsrChatClient::pbApplyServoArrayIfAny(uint32_t chunk_idx,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    /* 梯形曲线下最坏情况（1°/拍）的最短时长；与 PC 端预算逐位一致。 */
    const uint32_t physical_min_ms = head_servo_travel_min_ms(travel);
...
bool AsrChatClient::pbParseAndStage(const JsonDocument& doc) {
  ...
  /* 相机控制帧（仅 cam_fps / camera_once，无显示/舵机/音频通道）与 mic 提示帧同样走快速
   * 路径：它不占任何通道，不能进播放队列——否则卡通 9s 时间线常驻期间会被当成
   * "跨请求 append" 拒掉（drop chain head），拍照/预览失败；改成 replace 又会取消正在播的
   * 卡通序列。这里直接应用并回 played。 */
  const bool camera_once_early = doc["camera_once"] | false;
  const bool has_cam_control_early = camera_once_early || doc["cam_fps"].is<int>();
  if (type == "pb_single" && idx == 0u &&
      (mic_hint_early != PbMicHint::kNone || has_rtc_mode_early ||
       has_cam_control_early)) {
    ...
    if (next_bin_early == 0 && !pb_doc_has_nonempty_anim(doc) && !pb_doc_has_nonempty_servo(doc) &&
      ...
      if (camera_once_early) {
        camera_request_snapshot();
      } else if (doc["cam_fps"].is<int>()) {
        const int fps_early = doc["cam_fps"].as<int>();
        if (fps_early >= 0) {
          camera_set_fps((uint32_t)fps_early);
        }
      }
      ...
      log_info("[PB] media hint pb_single req=%s mic=%s rtc_interruptible=%d cam=%s",
               req.c_str(),
               mic_hint_early == PbMicHint::kOpen
                   ? "open"
                   : mic_hint_early == PbMicHint::kMute ? "mute" : "hold",
               (int)deskbot_uplink_rtc_interruptible(),
               camera_once_early ? "once"
                                 : (doc["cam_fps"].is<int>() ? "fps" : "-"));
...
void AsrChatClient::micGateReset() {
  mic_gate_open_until_ms_ = 0;
  mic_gate_was_open_ = false;
  mic_preroll_head_ = 0;
  mic_preroll_count_ = 0;
}

/* 能量 VAD：帧平均绝对值超过 max(绝对门限, 噪声底×倍数) 或 ESP-SR 报人声 → 开门 2 s。
 * 关门期间：更新噪声底（慢 EMA）、把帧存进预滚环、不编码不上行。
 * 开门瞬间：先把预滚里的 300 ms 编码送出，再送当前帧，保证第一个字不丢。 */
bool AsrChatClient::micGateShouldSend(const int16_t* frame, size_t samples, size_t abs_avg) {
  const uint32_t now = millis();
  const float threshold = mic_noise_floor_ * MIC_VAD_NOISE_RATIO;
  const bool loud = static_cast<float>(abs_avg) >
                    (threshold > static_cast<float>(MIC_VAD_ABS_MIN) ? threshold
                                                                     : static_cast<float>(MIC_VAD_ABS_MIN));
  if (loud || vad_gate_open_) {
    mic_gate_open_until_ms_ = now + MIC_VAD_HANGOVER_MS;
    if (mic_gate_open_until_ms_ == 0) mic_gate_open_until_ms_ = 1;
  }
  const bool open = mic_gate_open_until_ms_ != 0 &&
                    static_cast<int32_t>(mic_gate_open_until_ms_ - now) > 0;
  if (!open) {
    mic_noise_floor_ = mic_noise_floor_ * 0.98f + static_cast<float>(abs_avg) * 0.02f;
    if (mic_noise_floor_ < 30.0f) mic_noise_floor_ = 30.0f;
    if (mic_preroll_ != nullptr && samples == kFrameSamples20ms) {
      memcpy(mic_preroll_ + static_cast<size_t>(mic_preroll_head_) * kFrameSamples20ms, frame,
             kFrameSamples20ms * sizeof(int16_t));
      mic_preroll_head_ = static_cast<uint8_t>((mic_preroll_head_ + 1) % MIC_VAD_PREROLL_FRAMES);
      if (mic_preroll_count_ < MIC_VAD_PREROLL_FRAMES) mic_preroll_count_++;
    }
    if (mic_gate_was_open_) {
      mic_gate_was_open_ = false;
      (void)flushAudioOpusBatch();
    }
    mic_uplink_note_frame(false);
    return false;
  }
  if (!mic_gate_was_open_) {
    mic_gate_was_open_ = true;
    mic_uplink_note_gate_open();
    if (mic_preroll_ != nullptr && mic_preroll_count_ > 0) {
      uint8_t idx = static_cast<uint8_t>((mic_preroll_head_ + MIC_VAD_PREROLL_FRAMES - mic_preroll_count_) %
                                         MIC_VAD_PREROLL_FRAMES);
      for (uint8_t i = 0; i < mic_preroll_count_; ++i) {
        (void)queueAudioOpusFrame(mic_preroll_ + static_cast<size_t>(idx) * kFrameSamples20ms,
                                  kFrameSamples20ms);
        idx = static_cast<uint8_t>((idx + 1) % MIC_VAD_PREROLL_FRAMES);
      }
      mic_preroll_count_ = 0;
    }
  }
  mic_uplink_note_frame(true);
  return true;
}
...
bool AsrChatClient::runVoiceRound(uint16_t max_record_seconds) {
  ...
  if (!continuous_rollover) {
    ...
    micGateReset();
    if (mic_preroll_ == nullptr) {
      mic_preroll_ = static_cast<int16_t*>(heap_caps_malloc(
          static_cast<size_t>(MIC_VAD_PREROLL_FRAMES) * kFrameSamples20ms * sizeof(int16_t),
          MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      if (mic_preroll_ == nullptr) {
        log_warn("[MIC_UPLINK] preroll buffer unavailable; vad gate runs without preroll");
      }
    }
  } else {
  ...
  while (samples_recorded < total_samples && millis() < record_deadline_ms) {
    ...
    if (mic_uplink_muted()) {
      /* 静音开关：帧读出即丢——不增强、不编码、不上行，PC 收不到任何音频。
       * 0.0.54 的教训：这里 continue 之前必须照常跑 loopLite()，否则整轮录音（最长 30 s）
       * 期间 PB / 舵机 / 回执都没人服务，表情和张望要等到轮次结束才被处理。 */
      mic_uplink_note_frame(false);
      drainAudioFrontendVadEvents();
      loopLite();
      round_scope.pump("muted");
      taskYIELD();
      continue;
    }
    ...
    if (mic_uplink_mode() == MicUplinkMode::kVad &&
        !micGateShouldSend(frame, kFrameSamples20ms, frame_stats.abs_avg)) {
      /* 没人声：不编码不上行，只维护噪声底与预滚。 */
      round_scope.pump("gate_closed");
      taskYIELD();
      continue;
    }
    if (mic_uplink_mode() != MicUplinkMode::kVad) {
