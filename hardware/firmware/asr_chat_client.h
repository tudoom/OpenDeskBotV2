...
private:
  ...
  /* 设备端能量 VAD 门（mic_uplink_policy 模式为 vad 时生效）：
   * 门开到 gate_open_until_ms_，关门期间只更新噪声底、存 300 ms 预滚，不编码。 */
  uint32_t mic_gate_open_until_ms_ = 0;
  float mic_noise_floor_ = 200.0f;
  bool mic_gate_was_open_ = false;
  int16_t* mic_preroll_ = nullptr;
  uint8_t mic_preroll_head_ = 0;
  uint8_t mic_preroll_count_ = 0;
  bool micGateShouldSend(const int16_t* frame, size_t samples, size_t abs_avg);
  void micGateReset();
