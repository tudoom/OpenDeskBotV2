...
uint32_t age_ms(uint32_t now, uint32_t then) {
  ...
  const uint32_t delta = static_cast<uint32_t>(now - then);
  // 监督任务取完 now 快照后可能被抢占，期间被监控方又更新了时间戳
  // （then > now）。无符号下溢会把这台完全健康的设备伪装成停滞了
  // 49 天并立刻重启（实测 age=4294967295 的 mic_frame_stall 误杀）。
  return delta >= 0x80000000u ? 0 : delta;
}
...
/* 软复位归因：链接期 -Wl,--wrap=esp_restart 把所有 esp_restart 调用改道
 * 到这里，先把调用点返回地址存进 RTC 保持区再真正重启。配合 elf 的
 * addr2line 可解出精确调用行。充电器供电下崩溃重启不掉电，值可保持。 */
extern "C" {
void __real_esp_restart(void);
RTC_DATA_ATTR uint32_t g_deskbot_sw_restart_caller = 0;

void __wrap_esp_restart(void) {
  g_deskbot_sw_restart_caller =
...
uint32_t runtime_supervisor_sw_restart_caller(void) {
  return g_deskbot_sw_restart_caller;
}
