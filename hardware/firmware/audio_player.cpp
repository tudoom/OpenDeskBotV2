...
bool ensure_audio_play_task() {
  ...
  if (!s_audio_play_task) {
    /* 栈放 PSRAM（deskbot_task_create_pinned，失败回落内部堆）：audio_play
     * 只做 I2S 写、队列与 PB 终态事件，从不写 flash/NVS。0.0.46 以前已用
     * xTaskCreatePinnedToCoreWithCaps 走同一策略，这里只是统一入口。 */
    ...
    const BaseType_t rc = deskbot_task_create_pinned(
        ...
        &created_task, APP_CPU_NUM, &stack_in_psram);
    ...
    log_info(
        ...
        "memory=%s free_int=%u free_psram=%u",
        (int)AUDIO_PLAY_QUEUE_DEPTH, stack_in_psram ? "psram" : "internal",
        static_cast<unsigned>(heap_caps_get_free_size(
            MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)),
        static_cast<unsigned>(heap_caps_get_free_size(
            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)));
...
void audio_play_v2_startup_self_test(uint32_t duration_ms) {
  ...
  if (!audio_stream_pcm16_replace(0.1f)) {
    if (free_caps == 0) {
      free(pcm);
    } else {
      heap_caps_free(pcm);
  ...
  if (!audio_stream_pcm16_push_owned(
          pcm, kToneSamples, free_caps, 0.1f)) {
