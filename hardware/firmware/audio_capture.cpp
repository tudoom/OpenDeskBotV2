...
void task_setup_mic_capture() {
   ...
   * 栈放 PSRAM（deskbot_task_create_pinned，失败回落内部堆）：mic_cap 只碰
   * i2s 驱动、队列、原子计数和日志，从不写 flash/NVS，满足 PSRAM 栈前提；
   * 省下 8KB 内部 RAM 给相机驱动 / WiFi 收缓冲。 */
  ...
  log_info("[MIC] PDM new-channel capture started %uHz %u samp/frame "
           ...
           "queue_depth=%u queue_bytes=%u stack=%s",
