...
机器人不连接云服务器；默认通过 USB 直连，一键配网后也可经局域网 Wi-Fi 回连
PC（USB 优先）。联网、供应商 API、资源下载和时间同步都由 PC 负责。
...
1. 给所有机器人烧录同一份固件（出厂已烧录；升级可在控制台「固件」页一键完成）。
2. 启动 PC 服务并打开 `http://127.0.0.1:5050/`；页面直接进入本机控制台。
3. 用 USB 数据线连接机器人；`:9000` 自动扫描端口并完成 DBOT 握手。
4. 握手成功的连接自动出现在「设备连接」页。单台设备自动选中，多台设备可切换当前控制目标。
5. 现在即可使用对话、主动陪伴、提醒、记忆、表演和调试功能。
...
出厂默认为 DeepSeek `deepseek-flash`。在 Web 控制台填写，或在 `.env` 设置：
...
| 定时提醒 | 在「主动陪伴」里：每天 / 每周几 / 只提醒一次，到点由小歪主动开口，可先演一段表演；对小歪说「每天九点提醒我…」也会加进来 |
| 会话中心 | 本机唯一 `local` 会话的查看、清空和导出 |
| 勿扰与待机行为 | 勿扰时段、空闲张望都在「主动陪伴」页；音色在「声音」页 |
| 记忆 | `data/local/` 中共享的长期记忆 |
| 用量 | 这台 PC 的供应商调用汇总 |
...
| 路径 | 内容 |
|------|------|
| `data/opendesk.db` | 播放回执、工具/控制操作、陪伴进度、复刻任务与汇总用量 |
| `data/local/` | 这台 PC 唯一的会话、记忆、表情、定时提醒 / 陪伴场景、偏好和服务配置 |
...
```bash
python tools/network_connectivity_test.py --device-id deskbot_001122334455
```
