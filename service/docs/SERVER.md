...
| `/quest` | 主动陪伴：陪伴场景、定时提醒（每天 / 每周几 / 一次性）、勿扰时段与空闲张望 |
| `/sessions` | 会话中心 |
| `/memories` | 长期记忆 |
...
定时提醒随主动陪伴循环触发（没有独立调度器，见 ARCHITECTURE「定时提醒」）：

- 到点后 2 小时内补提；到点时在勿扰窗口的，勿扰结束后 2 小时内补提；
- 设备离线 / 通道忙时 15 秒后再试，宽限内都算；
- 今天叫过的记在 `data/local/quest/.proactive_state.json`，重启不重复；
- 一次性提醒提过即标记，不会明天再来。
...
python tools/network_connectivity_test.py \
  --device-id deskbot_001122334455
```
