...
- 设备与 PC 之间的主业务链路是 **USB CDC**；拔掉 USB 后同一 DBOT 协议改走 **WiFi TCP**（设备主动连 PC 的 9020），Core 侧由 `infrastructure/net/wifi_link.py` 监听、`SerialDeviceManager` 复用同一套会话机制。
...
- `play_expression`、`move_head`、相机、记忆、提醒、会话、
  米家、Web 和临时文件工具都能从 RTC 进入 Core；删记忆、写文件等风险确认仍由 Core
  强制执行，米家控制（写属性/调 action/跑场景）免确认，语音与文字一步到位。
...
（火山方舟）/ Xiaomi MiMo 三个预设并支持自定义 base URL；出厂默认为 DeepSeek
`deepseek-flash`，全部调用路径固定关闭深度思考（`thinking: disabled`）。
...
米家写操作通过 LLM 工具时免确认（产品决定，2026-09-02）：用户说"把台灯打开"
"用小爱播晴天"即直接执行。小爱音箱点歌/播电台/口令走 `execute-text-directive`
（args=[指令文本, true]），`play-text` 只会朗读文字。
...
设备生产数据走 USB CDC，或配网后由机器人回连的局域网 TCP（同一套 DBOT 协议，USB 优先）。
Core 不接收浏览器侧的音频、相机或 PB producer；`/camera_view` 和
`/device_pipeline?role=subscriber` 仅是本机调试订阅。
...
```text
hardware/.pio/build/deskbot_v2/firmware.bin
...
## 24. 2026-09 结构变更（阶段一～三）

本节记录 2026-09-02/03 的一轮全代码体检后落地的结构变更，读代码时以此为准：

### 24.1 唯一入口

| 概念 | 唯一实现 | 说明 |
| --- | --- | --- |
| LLM 工具执行 | `application/tool_executor.py` | `TOOL_CAPABILITIES` 是渠道 × 工具矩阵；`execute_tools_async`（Core/RTC）与 `execute_tools_sync`（Web）是仅有的两个入口，`execute_tools_round` / `execute_web_chat_tools` / RTC 允许列表都是它的薄封装或派生 |
| 系统提示装配 | `llm/prompt_assembly.py` | `assemble_text_system_prompt`（文字/传统链路）与 `assemble_rtc_system_prompt`（语音）；人设正文仍以 Agent.md 为准 |
| Core HTTP 路由 | `ws/routes/` | `ROUTES`（仅 loopback）与 `ROUTES_API_KEY`（过本机 API Key）两张表；新路由只登记模块，不再往 `http_api.py` 的 handler 里加 `if` |
| 固件版本 | `hardware/VERSION` | PlatformIO pre 脚本生成 `version_gen.h`；`service/scripts/sync_firmware_payload.py` 生成随包 manifest |
| 环境变量 | `config_registry.py` | 所有 `os.environ.get` 键必须登记（测试棘轮）；启动时打印非默认配置及来源（敏感值遮蔽） |
| 结构化遥测 | `telemetry.py` | `session.ready/closed`、`tool.exec`、`camera.snapshot` 等 JSON 行事件，落 `telemetry.jsonl`，`GET /api/telemetry/recent` 读取 |

### 24.2 拆分

- `web/blueprints/chat_bp.py`（文字对话）、`voice_clone_bp.py`（声音复刻）从 `debug_bp.py` 拆出。
- `application/expression_catalog.py`（表情目录/PB 帧构建，纯函数）从 `expression_runtime.py` 拆出，名字原样再导出。
- 固件 `pb_doc.{h,cpp}`：PB JSON 文档的纯解析/分类函数从 `AsrChatClient` 抽出；会话类只保留状态机。
...
- `ws/http_api.py` 仍有 23 个分支未搬进路由表（模式已就绪，逐个搬即可）。
- `AsrChatClient` 的 PB 播放状态机 / 传输会话 / 上行三段拆分需要真机回归，尚未动。
- 控制台并入 Core 进程（去掉 5050→9000 代理与自制 HTTP 栈）、VAD 替换（安装包 -80MB）需要产品与语音质量验证，未实施。
