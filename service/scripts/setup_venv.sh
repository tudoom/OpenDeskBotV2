...
configure_pip_index() {
  ...
  # 只影响本项目的 venv（--site），绝不改用户全局 pip 配置；已设置 PIP_INDEX_URL 的用户按自己的来。
  # 默认清华镜像是为国内网络提速，其它地区可 export PIP_INDEX_URL=https://pypi.org/simple 覆盖。
  if [[ -n "${PIP_INDEX_URL:-}" ]]; then
    return 0
  fi
  if ! "$py" -m pip config --site set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple 2>/dev/null; then
...
if [[ "$FAST_START" == "1" ]]; then
  ...
  if ! "$VPY" -c "import numpy, serial, websockets, yaml, webrtcvad, opuslib_next, croniter, miot, deskbot_server" >/dev/null 2>&1; then
  ...
  # miloco-miot（家居控制 SDK）没有 PyPI 发行版，wheel 随仓库提交，离线安装。
  # 不写进 pyproject/uv.lock 是刻意的：SDK 声明 av / fastmcp / paho-mqtt /
  # zeroconf 等重依赖，云端家居控制只用 miot.cloud / const / spec / storage /
  # types 五个模块，所以 --no-deps 装 wheel 本体即可（所需四个依赖已在主依赖中）。
  MILOCO_WHEEL="$(ls "$ROOT"/src/deskbot_server/iotctl/wheels/miloco_miot-*.whl 2>/dev/null | head -1 || true)"
  if [[ -n "$MILOCO_WHEEL" ]]; then
    miloco_wheel_ver="$(basename "$MILOCO_WHEEL" | cut -d- -f2)"
    miloco_installed_ver="$("$VPY" -c 'from importlib.metadata import version; print(version("miloco-miot"))' 2>/dev/null || true)"
    if [[ "$miloco_installed_ver" != "$miloco_wheel_ver" ]]; then
      echo "安装 miloco-miot ${miloco_wheel_ver}（家居控制 SDK，离线 wheel）..."
      "$VPY" -m pip install --disable-pip-version-check --no-deps --no-index --upgrade "$MILOCO_WHEEL"
    fi
  else
    echo "警告: 未找到 miloco-miot wheel（src/deskbot_server/iotctl/wheels/），家居控制不可用。" >&2
  fi
fi
