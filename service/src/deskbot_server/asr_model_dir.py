...
def asr_model_dir_ready(model_dir: str | Path) -> bool:
    ...
    for path in root.rglob("*"):
        if not path.is_file():
            continue
        # ModelScope writes an in-progress snapshot below hidden staging
        # directories such as ``._____temp``.  Treating a partially downloaded
        # ``._____temp/model.pt`` as ready makes the downloader skip forever
        # and the service then fails when FunASR tries to open the missing
        # committed root weight.
        try:
