...
def _load_miot_util():
    ...
    return MIOT_OK_CODES, infer_value


def execute_miot_tool(raw: dict[str, Any]) -> dict[str, Any]:
    """执行 LLM miot 工具调用。"""
    tool = "miot"
    ...
    try:
        MIOT_OK_CODES, infer_value = _load_miot_util()
    except RuntimeError as exc:
        return {"tool": tool, **ms.error_payload(exc, need_bind=False)}

    action = str(raw.get("action") or raw.get("op") or "list").strip().lower()
    try:
        if action in ("status", "auth_status"):
            st = ms.get_status(refresh=False)
            if st.get("bound"):
                st = ms.get_status(refresh=True)
        ...
        if action in ("list", "devices"):
            ...
            rows = ms.list_devices_cached_or_live(
                live=live, online_only=online_only
            )
            return {
                "tool": tool,
                "action": action,
        ...
        if action in ("scenes", "list_scenes"):
            cache = ms.load_homes_cache()
            scenes = list(cache.get("scenes") or [])
            if not scenes or raw.get("live"):
                scenes = ms._run(lambda s: s.list_scenes())
            return {
                "tool": tool,
                "action": action,
                "ok": True,
        ...
        if action in ("get", "device"):
            ...
            detail = ms._run(lambda s: s.get_device(dev["did"]))
        ...
        if action == "spec":
            ...
            spec = ms._run(lambda s: s.get_spec(dev["did"]))
            compact = []
        ...
        if action in ("props", "get_props", "properties"):
            ...
            if keys_raw is None or keys_raw == "":
                ...
                keys = [str(keys_raw)]
            results = ms._run(lambda s: s.get_properties(dev["did"], keys))
            ms.annotate_miot_results(results)
            failed = _first_failure(results, MIOT_OK_CODES)
            out: dict[str, Any] = {
                "tool": tool,
                "action": action,
                "ok": failed is None,
        ...
        if action in ("set", "set_prop", "write"):
            ...
            results = ms._run(lambda s: s.set_property(dev["did"], key, value))
            ms.annotate_miot_results(results)
            failed = _first_failure(results, MIOT_OK_CODES)
            out = {
                "tool": tool,
        ...
        if action in ("action", "call", "call_action"):
            ...
            result = ms._run(lambda s: s.call_action(dev["did"], key, args))
        ...
        if action in ("run_scene", "scene_run", "scene"):
            ...
            result = ms._run(lambda s: s.run_scene(str(sc["scene_id"])))
            return {
                "tool": tool,
                "action": action,
                "ok": True,
...
def _first_failure(
    ...
    ok_codes: set[int] | tuple[int, ...] | list[int],
) -> dict | None:
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        code = item.get("code")
        if isinstance(code, int) and code < 0 and code not in ok_codes:
            return item
    return None
