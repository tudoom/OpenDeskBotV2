...
def coerce_action_args(args: list[Any]) -> list[Any]:
    """动作入参里的布尔值转成 0/1。

    米家云端 ``/app/v2/miotspec/action`` 校验动作入参时不接受 JSON true/false
    （返回 -704220043「属性值不正确」），但接受 1/0。实测小爱音箱
    ``execute-text-directive`` 的 silent-execution：``[文本, true]`` 被拒，
    ``[文本, 1]`` 通过校验并下发到设备。模型/用户自然会写 true，这里统一折算。
    """
    return [int(a) if isinstance(a, bool) else a for a in list(args or [])]
