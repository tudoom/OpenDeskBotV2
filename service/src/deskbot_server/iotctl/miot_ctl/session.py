...
class MiotSession:
    def __init__(self, home: Path | str | None = None) -> None:
        ...
        def _initialize_config(cfg: dict) -> dict:
            cfg.setdefault("uuid", uuid.uuid4().hex)
            redirect_uri = str(cfg.get("redirect_uri") or "")
            if redirect_uri.rstrip("/") in (
                "https://127.0.0.1",
                "http://127.0.0.1",
            ):
                cfg["redirect_uri"] = "https://mico.api.mijia.tech/login_redirect"
            return cfg

        cfg = update_json(config_path(self._home), _initialize_config)
        self.uuid = cfg["uuid"]
    ...
    def _save_auth(self, info: MIoTOauthInfo, *, refreshed: bool = False) -> None:
        ...
        update_json(meta_path(self._home), _update_meta)

    def _clear_meta_bound(self) -> None:
        delete_json(meta_path(self._home))
        delete_json(homes_path(self._home))
    ...
    async def authorize(self, code: str, state: str) -> MIoTOauthInfo:
        ...
        try:
            ...
            logger.debug("authorize: swallowed exception", exc_info=True)
        self._save_auth(self._auth, refreshed=True)
        return self._auth

    async def unbind(self) -> None:
        self._auth = None
        delete_json(auth_path(self._home))
    ...
    async def call_action(self, did: str, key: str, args: list[Any]) -> dict:
        ...
        param = MIoTActionParam(
            did=did, siid=siid, aiid=aiid, in_=coerce_action_args(args)
        )
    ...
    async def list_scenes(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("list_scenes: swallowed exception", exc_info=True)
        rows = []
