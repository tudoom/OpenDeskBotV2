...
def build_rtc_mode_signal_pb(*, mode: str, req: str | None = None) -> dict[str, Any]:
    normalized = str(mode or "stable").strip().lower().replace("-", "_")
    if normalized not in {"stable", "interruptible", "esp32_aec"}:
        normalized = "stable"
    msg = build_mic_signal_pb(mic="hold", req=req)
    msg["rtc_mode"] = normalized
    return msg
