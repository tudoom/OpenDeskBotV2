...
from deskbot_server.pb.primitive_spec import (
    HEX6_BODY_PATTERN,
...
# 协议数据的单一来源是 ``pb.primitive_spec``；本模块保留旧名做兼容出口。
PB_DEFAULT_RGB565 = WIRE_DEFAULT_RGB565
PB_MAX_PRIMITIVES_PER_LAYER = MAX_PRIMITIVES_PER_LAYER
PB_LAYER_DEFAULT_COLORS: dict[str, str] = dict(LAYER_DEFAULT_COLORS)
PB_DEVICE_SHAPES = frozenset(SHAPE_CANONICAL)
_HEX_COLOR_RE = re.compile(HEX6_BODY_PATTERN)

# 配置源可写 ``c: "red"`` / ``color: "#F80"``；wire 统一下发 ``c``（RGB565 整数）
_NAMED_RGB888: dict[str, tuple[int, int, int]] = dict(NAMED_COLORS_RGB888)
...
# shape 主名与别名（小写键）→ 主名；单一来源 ``pb.primitive_spec.SHAPE_ALIASES``
_SHAPE_TO_CANONICAL: dict[str, str] = dict(SHAPE_ALIASES)
...
def normalize_primitive_for_wire(
    prim: dict[str, Any],
    *,
    layer: str | None = None,
) -> dict[str, Any]:
    ...
    out["shape"] = normalize_primitive_shape(str(out.get("shape") or ""))
    if out["shape"] not in PB_DEVICE_SHAPES:
        raise ValueError(f"unsupported device expression shape {out['shape']!r}")
    raw = out.get("c")
    if raw is None:
        raw = out.get("color")
    out.pop("color", None)
    default_color = PB_DEFAULT_RGB565
    if raw is None and layer in PB_LAYER_DEFAULT_COLORS:
        default_color = parse_color_to_rgb565(PB_LAYER_DEFAULT_COLORS[layer])
    out["c"] = parse_color_to_rgb565(raw, default=default_color)
    return out
...
def normalize_elements_for_wire(elements: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, val in (elements or {}).items():
        if isinstance(val, list):
            if len(val) > PB_MAX_PRIMITIVES_PER_LAYER:
                raise ValueError(
                    f"expression layer {key!r} exceeds "
                    f"{PB_MAX_PRIMITIVES_PER_LAYER} primitives"
                )
            out[key] = [
                normalize_primitive_for_wire(p, layer=key)
                for p in val
