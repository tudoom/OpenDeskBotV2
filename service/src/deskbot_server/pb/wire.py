...
def build_pb_wire_pairs(
    ...
    *,
    ...
    display_anim: bool = True,
) -> tuple[list[tuple[dict[str, Any], list[bytes]]], str, int, int]:
    """音素 TTS 分片 → pb wire (msg, binaries) 列表。

    ``display_anim=False``：音频分片不带显示层动画（口型）。屏幕上是位图（卡通）脸时用它——
    固件按"有无 anim"决定显示通道，纯音频 PB 不会把卡通脸顶掉；说话动画由表情运行时的
    卡通"说话"循环负责。LLM 显式 anims/images 仍照常下发（那是对话内容，不是口型）。
    """
    ...
    if not display_anim:
        # 位图脸下去掉显示层（口型/LLM anims），舵机步保留在 row["servo"] 里照常下发；
        # 带 moves 的手动 TTS 走 build_anim_rows_for_llm_plan 也必须剥掉，否则一句带动作的
        # 口播就把卡通脸顶成"只剩一张嘴"（2026-09-07 真机复现 /api/device_tts + moves）。
        for row in anim_rows:
            row["anim"] = []
        mouth_only = False
        logger.info(
            "[pb TX] 位图脸：音频分片不带口型动画 segments=%d moves=%d",
            len(anim_rows),
            len(move_steps),
        )
    pcm_list: list[bytes] = []
