...
可达性说明：默认链路（``deskbot-face.json`` 模板带 ``phonemes``）在
``phoneme_seq_to_anim_seq`` 里短路走设计文档，逐帧只写 mouth 层；本模块的
休眠层数据（``eye_l`` / ``eye_r`` / ``nose`` / ``extra`` 三态与
``metadata.blink`` 眨眼参数）仅在显式配置 ``DESKBOT_PB_FACE_BUNDLE`` /
``DESKBOT_PB_FACE_BUNDLE_JSON``（或 ``tts.pb_face_bundle*``）高级档时可达。
"""
...
_expr_default_bundle_cache: tuple[float, dict[str, Any]] | None = None
...
def load_expr_default_pb_face_bundle() -> dict[str, Any]:
    ...
    design_mtime = design_file_mtime()
    if (
        _expr_default_bundle_cache is not None
        and _expr_default_bundle_cache[0] == design_mtime
    ):
        return _expr_default_bundle_cache[1]
    out = expr_default_pb_face_bundle()
    _expr_default_bundle_cache = (design_mtime, out)
    _logger.info(
        "[pb_face_bundle] 已从本机 deskbot-face 加载 design_mtime=%s",
        design_mtime,
    )
    return out
...
def resolve_pb_face_bundle(tts_cfg: dict[str, Any] | None) -> dict[str, Any]:
    ...
    - 默认：本机 ``deskbot-face.json``（``emotions`` + ``phonemes``，按 mtime 热重载）。
