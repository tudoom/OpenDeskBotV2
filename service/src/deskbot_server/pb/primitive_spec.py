...
# 固件在 render runtime 创建失败的硬件兜底 idle 与「请先连接PC服务」待机屏
# 使用该几何；预览页 fallback 脸由此生成，保持三端同像素。
# 几何 = ``data/global/deskbot-face.json`` 系统 ``idle`` 场景第 0 帧（睁眼保持
# 帧）+ 控制台默认黄鼻子，使未连接时的待机脸与连接后的默认表情一致。
# 坐标系为 284×240 面板逻辑坐标；颜色为 RGB565：眼/嘴青 0x07FF、嘴缝黑
# 0x0000、鼻黄 0xFE87（#ffd23f），背景刷黑。
# ---------------------------------------------------------------------------
BUILTIN_FACE_SCREEN_BG_RGB565 = 0x0000
BUILTIN_FACE_RGB565 = 0x07FF
BUILTIN_NOSE_RGB565 = 0xFE87

BUILTIN_FACE_ELEMENTS: dict[str, list[dict[str, object]]] = {
    "bg": [],
    "nose": [
        {"shape": "circle", "x": 142, "y": 124, "r": 11, "sw": 1, "c": BUILTIN_NOSE_RGB565}
    ],
    "mouth": [
        {
            "shape": "round_rect",
    ...
    "eye_l": [
        {"shape": "ellipse_fill", "x": 105, "y": 96, "rw": 11, "rh": 11, "sw": 1, "c": BUILTIN_FACE_RGB565}
    ],
    "eye_r": [
        {"shape": "ellipse_fill", "x": 181, "y": 97, "rw": 11, "rh": 11, "sw": 1, "c": BUILTIN_FACE_RGB565}
    ],
    "extra": [],
}
