...
async def send_servo_moves_and_wait(
    ...
    *,
    ...
    built = build_servo_only_pb_payload(moves, request_id=request_id, level=level)
    if built is None:
    ...
    # 运动预算：装饰性动作超额直接丢，其它来源拉长时长。
    budgeted, decision = servo_budget().apply(device_id, list(payload.get("servo") or []), source=source)
    if not budgeted:
        return ServoMoveDelivery(
            request_id=req_id, delivered=0, accepted=False, played=False, status="budget_dropped"
        )
    if budgeted is not payload.get("servo"):
        payload["servo"] = budgeted
        payload["chunk_ms"] = sum(max(1, int(s.get("ms") or 0)) for s in budgeted)
    try:
