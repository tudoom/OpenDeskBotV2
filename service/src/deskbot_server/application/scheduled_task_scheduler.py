...
from deskbot_server.application import rtc_instructions
from deskbot_server.application.chat_flow import (
    _voice_was_played,
    publish_chat_turn,
    run_chat_turn,
)
from deskbot_server.application.scene_perform import run_scene_on_lane
...
class ScheduledTaskScheduler:
    ...
    async def _run_one(self, item: dict) -> None:
        ...
        try:
            ...
            if rtc_instructions.agent_attached(device_id):
                # 语音 Agent 在线：表演仍走设备 lane，提醒本身交给它说（进它的对话历史）
                async def _hand_over():
                    if scene_name:
                        try:
                            await run_scene_on_lane(
                                downlink, self._chat, device_id, scene_name, request_id=f"{req_id}_scene"
                            )
                        except asyncio.CancelledError:
                            raise
                        except Exception:  # noqa: BLE001
                            logger.warning("[scheduler] 提醒表演异常 task_id=%s scene=%s", tid, scene_name, exc_info=True)
                    rtc_instructions.enqueue(
                        device_id,
                        f"现在请你向主人朗声提醒并执行这件事：{description}。"
                        "直接说给主人听，一两句、口语，不要说「已发送」之类的汇报语。",
                        source="scheduled_task",
                    )

                try:
                    await device_turn_arbiter.run(
                        device_id, _hand_over, source="scheduled_task", priority=PRIORITY_REMINDER, preemptible=True
                    )
                except TurnInterrupted:
                    pass
                await asyncio.to_thread(
                    finish_scheduled_task, tid, ok=True, summary="已交给语音 Agent 播报", lease_token=lease_token
                )
                logger.info("[scheduler] 提醒已交给语音 Agent task_id=%s device_id=%s", tid, device_id)
                return

            async def _run_reminder_turn():
                if scene_name:
                    # 到点先演一段组合表演，再口播提醒；表演失败只记日志
                    try:
                        out = await run_scene_on_lane(
                            downlink, self._chat, device_id, scene_name, request_id=f"{req_id}_scene"
                        )
                        if not out.get("ok"):
                            logger.warning("[scheduler] 提醒表演失败 task_id=%s scene=%s err=%s", tid, scene_name, out.get("error"))
                    except asyncio.CancelledError:
                        raise
                    except Exception:  # noqa: BLE001
                        logger.warning("[scheduler] 提醒表演异常 task_id=%s scene=%s", tid, scene_name, exc_info=True)
