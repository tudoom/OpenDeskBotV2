...
from deskbot_server.application import user_activity
...
def _is_quest_proactive_user_text(user_text: str) -> bool:
    return str(user_text or "").strip().startswith(_QUEST_PROACTIVE_PREFIX)


def _quest_proactive_fallback_tts(user_text: str) -> str:
    """主动推进轮模型没开口/只写了汇报语时的兜底口播：按任务标题起个话头。"""
    first = str(user_text or "").strip().split("\n", 1)[0]
    # 取最后一个 [task_id] 之后的标题（前面的 [系统剧情推进] 前缀不算）
    m = re.search(r".*\[[^\]]+\]\s*([^\n]{1,40})$", first)
    title = (m.group(1).strip() if m else "").strip("：:。 ")
    if title and title != "notitle":
        return f"主人，我们聊聊{title}吧？"
    return "主人，我们聊聊吧？"
...
async def run_chat_turn(
    ...
    *,
    ...
    try:
        ...
        if not need_reply and not is_system_turn:
            tts_prefetch.cancel()
        ...
        if is_quest and (
            ...
            logger.info(
                "[quest_proactive] 主动轮使用兜底开口语 device_id=%s req=%s content=%s",
                device_id,
                request_id,
    ...
    try:
        async with downlink.pb_serial_chain():
            for chunk_i, chunk_text in enumerate(text_chunks):
                ...
                # 屏幕上是卡通（位图）脸时，音频分片不带口型动画：固件按"有无 anim"占用显示通道，
                # 带口型会把 JPEG 脸丢掉只剩一张嘴；说话动画交给表情运行时的卡通"说话"循环。
                bitmap_face = False
                try:
                    _face_runtime = get_expression_runtime(device_id)
                    bitmap_face = bool(_face_runtime is not None and _face_runtime.displayed_is_bitmap())
                except Exception:  # noqa: BLE001 - 查询失败按矢量脸处理
                    bitmap_face = False
