...
async def execute_tools_round(
    ...
    *,
    ...
    """Core 进程内一轮工具执行——薄封装，实现见 ``application.tool_executor``。"""
    from deskbot_server.application.tool_executor import ToolChannel, execute_tools_async

    return await execute_tools_async(
        tools,
        channel=ToolChannel.CORE,
        device_id=device_id,
