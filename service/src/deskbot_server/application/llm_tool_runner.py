...
from deskbot_server.application.quest_service import QUEST_TOOL_NAMES, execute_quest_tool
...
def _requires_explicit_confirmation(tool: str, raw: dict[str, Any]) -> bool:
    ...
    # 米家控制（开关灯、音箱点歌、跑场景）按产品决定免确认：用户说"把台灯打开"
    # 就直接执行，语音/文字都一步到位；删记忆、写文件等仍需明确确认。
    return False
    ...
    for raw in tools or []:
        ...
        try:
            if tool == "register_face":
                ...
                action = str(raw.get("action") or "read").strip().lower()
                if action == "read":
                    results.append(
                        {
                            "tool": tool,
                            "ok": True,
                            "action": action,
                            "text": agent_docs.read_doc(agent_docs.USER_FILENAME),
                        }
                    )
                elif action in ("append", "replace"):
                    text = str(raw.get("text") or "").strip()
                    if not text:
                        raise ValueError("user_note 需要 text")
                    if action == "append":
                        # 追加而不是整篇重写：模型只学到一件事时不该有机会
                        # 顺手抹掉之前积累的全部理解。
                        current = agent_docs.read_doc(
                            agent_docs.USER_FILENAME
                        ).rstrip()
                        body = f"{current}\n- {text}\n" if current else f"- {text}\n"
                    else:
                        body = text
                    saved = agent_docs.write_doc(agent_docs.USER_FILENAME, body)
                    results.append(
                        {"tool": tool, "ok": True, "action": action, **saved}
                    )
                else:
                    raise ValueError(f"unknown user_note action: {action}")
            elif tool == "memory_add":
                text = str(raw.get("text") or raw.get("value") or "").strip()
                if not text:
                    raise ValueError("memory_add 需要 text")
                # 称呼类记忆：只有一个字/像截断的半句先不记，让小歪反问确认；记成功后旧称呼作废。
                address_name = extract_address_name(text)
                if address_name is not None and name_is_suspicious(address_name):
                    raise ValueError(address_confirmation_hint(address_name))
                entry = add_memory(text)
                superseded: list[str] = []
                if address_name is not None:
                    for old_id in supersede_address_memories(list_memory_entries(), str(entry["id"])):
                        if delete_memory(old_id):
                            superseded.append(old_id)
                result = {"tool": tool, "ok": True, "id": entry["id"], "text": entry["text"]}
                if superseded:
                ...
                results.append(out)
            elif tool in QUEST_TOOL_NAMES:
                results.append(execute_quest_tool(raw, device_id=dev))
