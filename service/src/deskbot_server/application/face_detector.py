...
# 人脸检测栈的顶层模块。默认客户端安装包不携带它们（Build-Client.ps1 未加
# -IncludeFaceStack 时会从 site-packages 删除；pip 侧对应 optional extra
# ``deskbot-server[face]``）。缺失时人脸检测/识别明确降级，相机预览与拍照
# 问答不受影响。
_FACE_STACK_MODULES = ("mediapipe", "cv2")


def face_stack_available() -> bool:
    """人脸检测栈（mediapipe/cv2）是否随本安装提供。

    仅探测 module spec，不导入重库，可在任意线程（含 Web 请求线程）低成本
    调用。spec 查找本身抛 ImportError/ValueError 也视为不可用（例如打包
    残缺或测试环境用 meta-path 拦截器模拟缺包）。
    """
    import importlib.util

    for name in _FACE_STACK_MODULES:
        try:
...
class CameraFaceDetector:
    ...
    def detect_faces(
        ...
        image_bytes: bytes,
        *,
        decoded_rgb: Optional[np.ndarray] = None,
    ) -> list[dict[str, Any]]:
        """检测帧内最多 ``num_faces`` 张人脸，返回未带 ``face_id`` 的原始结果列表。

        ``decoded_rgb`` 允许调用方复用入口校验（``validate_jpeg_with_rgb``）已
        解码的同帧像素，避免同一 JPEG 被解码两次；缺省时保持独立解码能力。
        """
        if self._closed:
            return []
        rgb = (
            ...
            else self._decode_jpeg_to_rgb(image_bytes)
        )
