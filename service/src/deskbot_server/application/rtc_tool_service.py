...
from deskbot_server.application.tool_executor import ToolChannel, tools_for_channel  # noqa: E402

# 允许列表由渠道能力矩阵导出：新增工具只在 tool_executor.TOOL_CAPABILITIES 登记。
_RTC_TOOL_NAMES = tools_for_channel(ToolChannel.RTC)
_CORE_TOOL_NAMES = _RTC_TOOL_NAMES - {"move_head", "play_expression"}
_RTC_VISION_IMAGE_B64_KEY = "_rtc_vision_image_b64"
_MOVE_MS_MIN = 200
_MOVE_MS_MAX = 8000
# 与工具 schema 的 maxItems 对齐：五步够表达点头、张望这类动作，
# 再长就该沉淀成预设，而不是每次让模型现编。
_MOVE_MAX_COMPOSED_STEPS = 5
...
async def _move_head_composed(
    *,
    ...
    """Play a motion the model composed from atomic steps."""
    if len(steps) > _MOVE_MAX_COMPOSED_STEPS:
        return {
            "tool": "move_head",
            "ok": False,
            "status": "invalid_move",
            "error": (
                f"composed motion is limited to {_MOVE_MAX_COMPOSED_STEPS} steps"
            ),
        }
    ...
    for index, raw in enumerate(steps):
        if not isinstance(raw, dict):
            return {
                ...
                "ok": False,
                "status": "invalid_move",
                "error": f"composed step {index} must be an object",
            }
        step_ms = _bounded_int(
            raw.get("ms"),
            minimum=SERVO_MIN_SEGMENT_DURATION_MS,
        ...
        moves.append(
            {
                # __custom__ 是协议层给显式步骤留的通道：它照样走限位、
                # 包络与分片，只是不查预设表。
                "move": "__custom__",
                "ms": step_ms,
                "x": raw.get("x", 90),
    ...
    if total_ms > _MOVE_MS_MAX:
        return {
            "tool": "move_head",
            "ok": False,
            "status": "invalid_move",
            "error": f"composed motion exceeds {_MOVE_MS_MAX} ms",
        }
    stable_request_id = None
    if str(call_id or "").strip():
        stable_request_id = "rtcc" + hashlib.sha256(
    ...
    try:
        delivery = await send_servo_moves_and_wait(
            ...
            device_id,
            moves,
            source="rtc_tool_move_composed",
            summary=f"RTC composed move ({len(moves)} steps)",
        ...
        return {
            ...
            "ok": False,
            "status": "invalid_move",
            "error": "composed motion cannot be represented safely",
        }
    return {
        "tool": "move_head",
        "ok": delivery.ok,
...
async def _move_head(
    *,
    ...
    if not move and isinstance(raw_steps, list) and raw_steps:
        # 库里没有合适的动作时，模型可以用原子步骤自己编排一个。校验、
        # 限位和分片仍由协议层做，所以编错了只会被拒绝，不会伤到硬件。
        return await _move_head_composed(
            device_id=device_id,
            steps=raw_steps,
    ...
    if not move:
        return {
            "tool": "move_head",
            "ok": False,
            "error": "move_head needs either a configured preset or composed steps",
        }
    try:
