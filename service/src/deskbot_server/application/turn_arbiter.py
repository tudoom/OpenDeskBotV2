...
class DeviceTurnArbiter:
    ...
    def __init__(self) -> None:
        ...
        # 每台设备最近一次跑完的 lane 动作：{device_id: {source: ts}}。
        # 主动陪伴据此判断「空闲」——对话、表情、动作、表演、提醒都算和主人有过交互。
        self._last_activity: dict[str, dict[str, float]] = {}

    def last_activity_ts(self, device_id: str, *, exclude: tuple[str, ...] = ()) -> float:
        rows = self._last_activity.get(str(device_id or ""), {})
        return max((ts for src, ts in rows.items() if src not in exclude), default=0.0)
    ...
    def _finish(self, entry: _Entry) -> None:
        self._last_activity.setdefault(entry.device_id, {})[entry.source] = time.time()
        states = self._states()
