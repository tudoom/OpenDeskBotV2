(function (global) {
  ...
  function shapeToSvg(p, layer) {
    ...
    if (shape === "image") {
      const idx = Math.max(0, Math.trunc(num(p.asset, 0)));
      const b64 = currentSceneAssets[idx];
      if (!b64) return "";
      const href = String(b64).startsWith("data:") ? b64 : `data:image/jpeg;base64,${b64}`;
      return `<image href="${href}" x="${num(p.x, 0)}" y="${num(p.y, 0)}" width="${Math.max(1, num(p.w, 1))}" height="${Math.max(1, num(p.h, 1))}" preserveAspectRatio="none"${tr}/>`;
    }
    if (shape === "hline") {
      const x = num(p.x, 0);
      const y = num(p.y, 0);
  ...
  // 卡通表情：场景级 assets（base64 JPEG），image 图元按 asset 下标引用
  let currentSceneAssets = [];
  function sceneToSvg(scene, frameIndex, progress) {
    currentSceneAssets = scene && Array.isArray(scene.assets) ? scene.assets : [];
