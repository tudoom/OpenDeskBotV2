(function () {
  ...
  function renderOtherTabNotice() {
    ...
    const microphoneHealth =
      ...
      typeof state.target.microphone_health === "object"
        ? state.target.microphone_health
        : null;
    // 麦克风健康告警只在首页「Agent 全面日志」内展示，不再挂全局横幅。
    void microphoneHealth;
    if (
      !otherTabDeviceId ||
      !state.targetId ||
  ...
  async function refresh() {
    ...
    try {
      const payload = await apiJson(DEVICE_API, { timeoutMs: 8000 });
      ...
      // 核心服务忙（语音播报/相机预览）时单次超时很常见：已有可用状态则
      // 先按旧数据展示，连续两次失败才对用户宣告「控制台状态不可用」。
      if (state.phase === "ready" && refreshFailureStreak < 2) {
        state = Object.assign({}, state, { generation: seq });
      } else {
