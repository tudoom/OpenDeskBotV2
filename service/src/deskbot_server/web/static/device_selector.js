(function () {
  ...
  function initDeviceSelector(root) {
    ...
    // 片内温度告警：任何页面顶部都能看到（数据来自 /app/api/devices 的 thermal 字段）。
    function renderThermalNotice() {
      const el = document.getElementById("globalDeviceNotice");
      if (!el) return;
      const hot = devices.filter((d) => d && d.thermal && (d.thermal.level === "warn" || d.thermal.level === "crit"));
      if (!hot.length) { el.hidden = true; el.textContent = ""; el.classList.remove("crit"); return; }
      const worst = hot.sort((a, b) => (b.thermal.temp_c || 0) - (a.thermal.temp_c || 0))[0];
      const name = worst.display_name || worst.device_id;
      el.textContent = (worst.thermal.level === "crit" ? "🔥 " : "⚠ ") + name + "：" + (worst.thermal.message || ("芯片 " + worst.thermal.temp_c + "℃"));
      el.classList.toggle("crit", worst.thermal.level === "crit");
      el.hidden = false;
    }
