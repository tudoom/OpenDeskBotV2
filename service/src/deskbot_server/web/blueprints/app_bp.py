...
_snapshot_cache: dict = {"at": 0.0, "live": None, "usb": None}
_snapshot_cache_lock = threading.Lock()


def invalidate_upstream_snapshot_cache() -> None:
    """清掉 2s 微缓存。应用创建时调用：进程内重建 app（测试）不得看到上一个
    app 实例的上游观测。"""
    with _snapshot_cache_lock:
        _snapshot_cache.update(at=0.0, live=None, usb=None)


def _fetch_upstream_snapshots() -> tuple[dict, dict]:
    """两个上游快照并行取 + 2s 微缓存。

    core 忙（语音播报/相机预览）时单次上游调用可达 1s+，串行两次再乘上
    切 tab 的突发请求，会把 /app/api/devices 顶过浏览器 5s 超时——顶栏
    闪「控制台状态不可用」且整页跟着卡。缓存只盖上游观测，session 相关
    的 current 选择仍每请求计算。"""
    now = time.monotonic()
    with _snapshot_cache_lock:
        if (
            _snapshot_cache["live"] is not None
            and now - _snapshot_cache["at"] < _SNAPSHOT_CACHE_TTL_S
        ):
            return _snapshot_cache["live"], _snapshot_cache["usb"]
    with ThreadPoolExecutor(max_workers=2) as pool:
        live_future = pool.submit(fetch_live_device_snapshot)
        usb_future = pool.submit(fetch_attached_usb_snapshot)
        live_snapshot = live_future.result()
        usb_snapshot = usb_future.result()
    with _snapshot_cache_lock:
        _snapshot_cache.update(
            at=time.monotonic(), live=live_snapshot, usb=usb_snapshot
        )
    return live_snapshot, usb_snapshot


def _local_device_snapshot():
    """Return the physical USB connection directory for this PC."""

    live_snapshot, usb_snapshot = _fetch_upstream_snapshots()
        ...
        rows.append(
            {
                ...
                "camera_health": live.get("camera_health"),
                "transport": (
...
@bp.get("/api/agent/docs")
def api_agent_docs():
    """The three Markdown documents shown on the Agent 伙伴 page."""
    from deskbot_server import agent_docs

    return jsonify({"ok": True, "docs": agent_docs.list_docs()})


@bp.get("/api/agent/docs/<name>")
def api_agent_doc_read(name: str):
    from deskbot_server import agent_docs

    try:
        text = agent_docs.read_doc(name)
    except ValueError as exc:
        return jsonify({"ok": False, "error": str(exc)}), 404
    return jsonify({"ok": True, "name": name, "text": text})


@bp.post("/api/agent/docs/<name>")
def api_agent_doc_write(name: str):
    from deskbot_server import agent_docs

    payload = request.get_json(silent=True) or {}
    try:
        saved = agent_docs.write_doc(name, payload.get("text") or "")
    except ValueError as exc:
        return jsonify({"ok": False, "error": str(exc)}), 400
