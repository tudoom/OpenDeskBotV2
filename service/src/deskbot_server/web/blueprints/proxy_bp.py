...
_PROXY_METHODS: dict[str, frozenset[str]] = {
    ...
    "/api/face_status": frozenset({"GET"}),
    "/api/face_persist": frozenset({"POST"}),
    "/api/face_clear": frozenset({"POST"}),
    ...
    # 首页相机预览：每 10 秒抓一帧（image/jpeg 透传），不再常驻取帧流。
    "/api/camera_snapshot": frozenset({"GET"}),
    # 结构化遥测最近事件（首页面板）
    "/api/telemetry/recent": frozenset({"GET"}),
    "/api/servo_contract": frozenset({"GET"}),
    "/api/servo_config": frozenset({"GET", "POST"}),
    # 舵机到位释放开关（实验室页）
    "/api/servo_relax": frozenset({"GET", "POST"}),
    # 省电与保护（实验室页）：麦克风按需上行、温度断电阈值
    "/api/mic_uplink_mode": frozenset({"GET", "POST"}),
    "/api/mic_mute": frozenset({"GET", "POST"}),
    "/api/live_behavior": frozenset({"GET"}),
    "/api/thermal_cutoff": frozenset({"GET", "POST"}),
    ...
    # 主动陪伴：循环状态 / 现在试一句（Core 进程内才有循环）
    "/api/quest_proactive": frozenset({"GET"}),
    "/api/quest_proactive/speak_now": frozenset({"POST"}),
    "/api/wifi_status": frozenset({"GET"}),
    "/api/wifi_provision": frozenset({"POST"}),
    "/api/rtc_health": frozenset({"GET"}),
    "/api/firmware_manifest": frozenset({"GET"}),
    "/api/firmware_update": frozenset({"POST"}),
    "/api/firmware_update_status": frozenset({"GET"}),
}
