...
class FrameDecoder:
    ...
    def __init__(self, *, max_payload: int = MAX_PAYLOAD) -> None:
        ...
        self.last_discarded_sample: bytes = b""
        self.last_crc_failure: dict | None = None
    ...
    def _discard_prefix(self, count: int) -> None:
        if count <= 0:
            return
        # Keep a short sample of what was thrown away so the session can show
        # whether the byte stream carried foreign text (unframed device logs)
        # or merely a corrupted frame.
        if count >= 8:
            self.last_discarded_sample = bytes(self._buffer[: min(count, 96)])
    ...
    def feed(self, data: bytes | bytearray | memoryview) -> list[Frame]:
        ...
        while True:
            ...
            if crc32(payload) != payload_crc:
                ...
                # Diagnostic: where is the next magic relative to the declared
                # frame end?  ==0 means same-length corruption, <0 bytes were
                # lost (next frame starts early), >0 bytes were inserted.
                next_magic = self._buffer.find(MAGIC_BYTES, 1)
                self.last_crc_failure = {
                    "channel": int(channel),
                    "sequence": int(sequence),
                    "payload_len": int(payload_len),
                    "frame_size": int(frame_size),
                    "next_magic_delta": (
                        int(next_magic - frame_size) if next_magic >= 0 else None
                    ),
                    "buffered": len(self._buffer),
                    "head_hex": bytes(self._buffer[:32]).hex(),
                    "tail_hex": bytes(
                        self._buffer[max(0, payload_end - 16) : payload_end + 8]
                    ).hex(),
                }
