...
from deskbot_server.application.core_binding_refresh import (
    REFRESH_DELAY_SEC,
    desired_binding,
...
class SerialServiceBridge:
    ...
    async def _refresh_core_binding(self, session: DeviceSession) -> None:
        if not wifi_link_enabled():
            return
        try:
            await asyncio.sleep(REFRESH_DELAY_SEC)
            if session.is_closed:
                return
            host = await asyncio.to_thread(pc_lan_ip)
            desired = desired_binding(
                host=host, port=wifi_link_port(), core_id=core_id(), token=""
            )
            result = await refresh_core_binding(
                session,
                desired=desired,
                token_provider=lambda dev: asyncio.to_thread(ensure_link_token, dev),
            )
            if result.get("action") != "skipped" or result.get("reason") not in {
                "up_to_date",
                "firmware_too_old",
                "unprovisioned",
            }:
                logger.info(
                    "[core_binding] device_id=%s %s (%s)",
                    session.device_id,
                    result.get("action"),
                    result.get("reason"),
                )
            ...
            raise
        except Exception:  # noqa: BLE001 - 刷新失败不影响会话
            logger.debug("[core_binding] refresh failed", exc_info=True)
    ...
    async def _on_ready(
        ...
        handler = asyncio.create_task(
            handle_asr_chat(
                ...
                api_key_id=None,
                registry_channel=session.transport,
                registry_transport=session.transport,
        ...
        # 固件 ≥0.0.52 的 hello 带片内温度：会话一建立就记一笔（心跳后续由观察者接手）。
        try:
            if int(getattr(hello, "chip_temp_c", 0) or 0) > 0:
                thermal_guard().observe(hello.device_id, hello.chip_temp_c, transport=session.transport)
        except Exception:  # noqa: BLE001
            logger.debug("[thermal] hello observe failed", exc_info=True)
        # USB 会话里把 Core 当前地址/身份刷给设备（固件 ≥0.0.49，host-only、
        # 不重启）。必须是独立任务：本回调跑在会话的 RX 循环上，等回执会死锁。
        if session.transport == "usb_cdc":
            refresh = asyncio.create_task(
                self._refresh_core_binding(session),
                name=f"deskbot-core-binding:{hello.device_id}:{session.generation}",
            )
            self._binding_tasks.add(refresh)
            refresh.add_done_callback(self._binding_tasks.discard)
