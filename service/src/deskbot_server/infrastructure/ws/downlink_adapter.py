...
class WsDownlinkAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        # The USB bridge intentionally reuses the websocket-facing application
        # flow, so HTTP controls such as /api/device_tts receive a DeviceSession
        # wrapped by this adapter.  Enable the media mic barrier only for that
        # physical CDC transport; ordinary websocket connections remain
        # full-duplex.
        self.half_duplex_media_mic = isinstance(websocket, DeviceSession)
    ...
    async def cancel_pb_playback(self, request_id: str) -> bool:
        """Abort queued PB work and send an out-of-band device cancel."""

        return await cancel_pb_device_downlink(
            self._ws,
            device_pb_json_msg({"type": "pb_cancel", "req": request_id}),
        )
