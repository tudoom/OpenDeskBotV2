...
from deskbot_server.asr_model_dir import asr_model_dir_ready, has_quant_onnx
...
class FunAsrAdapter:
    ...
    def __init__(self, settings: AppSettings) -> None:
        ...
        self._language = (
            os.environ.get("ASR_LANGUAGE") or settings.asr.language or "zh"
        ).strip()
        ...
        if self._use_quant_onnx:
            if has_quant_onnx(model_dir):
                ...
                logger.warning(
                    "[ASR] model_quant.onnx 不可用，回退 PyTorch model.pt；"
                    "运行时不会自动导出模型 model_dir=%s",
                    model_dir,
                )
    ...
    def _validate_model_dir(model_dir: str) -> None:
        if not model_dir or not os.path.isdir(model_dir):
            raise ValueError(
                f"本地 ASR 模型目录不存在：{model_dir or '(未配置)'}。"
                "请自行准备 SenseVoiceSmall 模型，设置 "
                "ASR_MODEL_DIR，并显式配置 ASR_PROVIDER=funasr；"
                "服务不会自动下载模型。"
            )
        if asr_model_dir_ready(model_dir):
            return
        raise ValueError(
            "本地 ASR 模型目录缺少 model.pt 或 model_quant.onnx："
            f"{model_dir}。请自行放置完整 SenseVoiceSmall 模型；"
            "服务不会自动下载或导出权重。"
        )
