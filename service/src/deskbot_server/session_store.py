...
def execute_session_tool(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if action in {"search", "find", "recall"}:
        from deskbot_server.session_search import search as search_sessions

        query = str(raw.get("query") or raw.get("q") or raw.get("text") or "").strip()
        if not query:
            raise ValueError("session search needs a query")
        hits = search_sessions(
            query,
            limit=int(raw.get("limit") or raw.get("max") or 20),
            role=str(raw.get("role") or "").strip().lower() or None,
        )
        return {
            "tool": "session",
            "action": "search",
