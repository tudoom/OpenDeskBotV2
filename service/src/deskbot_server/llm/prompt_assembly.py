...
_RTC_BEHAVIOR_RULES = (
    ...
    "主人让你编一段新表演、设计一个新动作、做一套新卡通表情、画一个表情、安排一个陪伴计划时，"
    "分别调用 generate_scene、generate_motion、generate_cartoon_faces、generate_expression、"
    "generate_quest_scene 直接去做，不要说做不到或让主人去网页操作；"
    "卡通套图要三四分钟，调用后先告诉主人在做了、做好会主动说，不要重复调用。"
)
