...
New installations default to Xiaomi MiMo's OpenAI-compatible endpoint.  The
legacy Volcengine Ark environment aliases remain supported for existing
deployments.
"""
...
# Backwards-compatible import name; the product's OpenAI-compatible default is
# DeepSeek (2026-09-11 用户决定：出厂默认 DeepSeek deepseek-flash，此前是 MiMo mimo-v2.5).
OPENAI_BASE_URL = DEEPSEEK_OPENAI_BASE_URL
...
def _default_base_url(protocol: str, *, api_key_source: str | None = None) -> str | None:
    ...
    if protocol == "openai" and api_key_source in {"VOLCENGINE_API_KEY", "DOUBAO_API_KEY"}:
        return ARK_OPENAI_BASE_URL
    if protocol == "openai":
...
def resolve_system_llm_config(*, require_model: bool = True) -> ResolvedLlmConfig:
    """Resolve the system LLM configuration.

    Runtime callers keep the default strict behavior so a missing model fails
    before any provider request is attempted.  Configuration and onboarding
    screens may opt into a non-throwing snapshot while the operator is still
    ...
    protocol = _normalized_protocol(
        ...
        or ("ark" if api_key_source in {"VOLCENGINE_API_KEY", "DOUBAO_API_KEY"} else None)
        or "openai"
    )
    model_name = str(
    ...
    # An explicitly empty LLM_BASE_URL means "use this protocol's default".
    # This differs from an absent key, which still falls back to legacy aliases
    # and config.yaml for backwards compatibility.
    ...
    resolved_model = (
        build_chat_model(protocol, model_name)
        if require_model or model_name
        else ""
    )
    display_name = (
        f"系统默认 ({model_name})"
...
def build_provider_auth_headers(api_url: str, api_key: str) -> dict[str, str]:
    """Return the provider-specific HTTP authentication header.

    Xiaomi MiMo's official examples document ``api-key``; emit that form for
    its public origin.  Other compatible providers keep the conventional
    ``Authorization: Bearer`` form.
    """

    hostname = (urllib.parse.urlsplit(str(api_url or "")).hostname or "").lower()
    if hostname.rstrip(".") == "api.xiaomimimo.com":
        return {"api-key": api_key}
    return {"Authorization": f"Bearer {api_key}"}


def _messages_to_ark_input(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """OpenAI ``messages`` → 火山方舟 Responses API ``input``。"""
    out = to_ark_responses_input(messages)
    if not out:
...
def _validate_api_key(cfg: ResolvedLlmConfig) -> None:
    if not cfg.api_key or "请替换" in cfg.api_key:
        raise ValueError(
            ...
            "LLM_API_KEY / VOLCENGINE_API_KEY / DOUBAO_API_KEY / "
...
def _build_completion_payload(
    ...
    *,
    ...
    supported_vision_protocols = OPENAI_COMPAT_PROTOCOLS | ARK_RESPONSES_PROTOCOLS
    if has_images and _normalized_protocol(cfg.protocol) not in supported_vision_protocols:
        raise LlmVisionUnsupportedError(
            "当前 LLM 协议不支持图像输入。请在 PC「高级设置」中切换到"
            "支持视觉的 OpenAI-compatible 或 Ark Responses 模型后重试。"
        )
    ...
    payload = {
        "model": build_chat_model(cfg.protocol, cfg.model),
        "messages": to_openai_chat_messages(messages),
...
def _provider_http_error(
    ...
    *,
    ...
    """Translate provider rejections without ever including image payloads."""

    if messages_contain_images(messages):
        try:
            exc.close()
        except Exception:
            logger.debug("_provider_http_error: swallowed exception", exc_info=True)
        if int(exc.code) in {400, 404, 413, 415, 422}:
            return LlmVisionUnsupportedError(
                "当前 LLM 模型或接入点拒绝了图像输入 "
                f"(HTTP {exc.code})。请在 PC「高级设置」中切换到支持视觉的模型，"
                "并确认该模型的 Ark/OpenAI-compatible 接口已启用图片能力后重试。"
            )
        return RuntimeError(
            f"LLM 视觉请求失败 HTTP {exc.code}。请检查模型服务凭证、额度和可用性后重试。"
        )
    try:
    ...
    label = "LLM SSE 读取失败" if stream else "LLM API 请求失败"
    return RuntimeError(f"{label} HTTP {exc.code}: {preview}")
...
def _request_chat_completion_stream(
    ...
    *,
    ...
    req = urllib.request.Request(
        ...
        data=body,
        headers={
            **build_provider_auth_headers(url, cfg.api_key),
    ...
    try:
        resp = safe_provider_urlopen(req, timeout=timeout)
...
def _request_chat_completion(
    ...
    *,
    ...
    try:
        ...
        preview = (
            "<visual response redacted>"
            if messages_contain_images(messages)
            else raw[:1000].decode("utf-8", "replace")
        )
...
async def chat_acompletion(
    ...
    *,
    ...
    if use_stream:
        ...
        # ``asyncio.to_thread`` cancellation cannot stop the underlying HTTP
        # request.  Treat the first-token threshold as a slow-provider
        # diagnostic and keep awaiting the request so that a delayed HTTP 401,
        # quota rejection, or transport error is not replaced by a generic
        # five-second timeout.
        if first_token_timeout > 0:
            ...
            if not token_wait_task.done():
                ...
                try:
                    ...
                    pass
            # 超过阈值时保留请求；供应商的真实结果/错误由下方 await 传播。
            if stream_task not in done and not first_token_event.is_set():
                logger.warning(
                    "[LLM] 首字等待超过 %.1fs，继续等待供应商响应 device_id=%s",
                    first_token_timeout,
                    device_id,
                )
