...
_SENSITIVE_DEBUG_HEADERS = frozenset(
    {
        "authorization",
        "cookie",
        "proxy-authorization",
...
def _debug_request_path(path: str) -> str:
    """Never echo query credentials into protocol debug logs."""

    return str(path or "").partition("?")[0]


def _debug_header_value(key: str, value: str) -> str:
    name = str(key or "").strip().lower()
    if (
        name in _SENSITIVE_DEBUG_HEADERS
        or "token" in name
    ...
    if self.state is CONNECTING:
        ...
        if self.debug:
            ...
            self.logger.debug(
                "< %s %s HTTP/1.1",
                method,
                _debug_request_path(request.path),
            )
            for key, value in request.headers.raw_items():
                self.logger.debug(
                    "< %s: %s",
                    key,
                    _debug_header_value(key, value),
                )
