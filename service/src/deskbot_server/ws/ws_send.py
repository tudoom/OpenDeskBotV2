    ...
    *,
    ...
    try:
        ...
        try:
            ...
            logger.debug("_safe_send_once: swallowed exception", exc_info=True)
        try:
...
def _sender_accepts_binaries(sender) -> bool:
    try:
        params = inspect.signature(sender).parameters
    except (TypeError, ValueError):
        return False
    return "binaries" in params
...
async def _safe_send_pb_json_then_binaries(
    ...
    *,
    ...
    bundle = getattr(websocket, "send_pb_wire", None)
    if binaries and callable(bundle) and _sender_accepts_binaries(bundle):
        # USB 串口会话：JSON + 全部二进制在会话的 bundle 锁里原子发出（并发的
        # 表情 PB 不能插在中间；发送方被取消也由会话把已声明的二进制补完）。
        total_timeout = max(
            float(timeout),
            sum(_send_timeout_for_message(b, base=timeout) for b in binaries),
        )
        async with _get_ws_send_lock(websocket):
            try:
                await asyncio.wait_for(
                    bundle(text_msg, binaries=list(binaries)),
                ...
                logger.warning(
                    "[pb TX] bundle 下发失败 peer=%s bins=%d err=%s",
                    _peer_str(websocket),
                    len(binaries),
                    exc,
                )
...
def _clear_pb_device_worker_state(ws, task, q) -> None:
    ...
    if getattr(ws, _PB_DEVICE_WORKER_ATTR, None) is task:
        try:
            ...
            logger.debug("_clear_pb_device_worker_state: swallowed exception", exc_info=True)
...
async def _stop_pb_device_downlink_worker(ws) -> None:
    ...
    try:
        ...
        logger.debug("_stop_pb_device_downlink_worker: swallowed exception", exc_info=True)
    ...
    if q is not None:
        ...
        if task is current:
            ...
            try:
                ...
                logger.debug("cancel_pb_device_downlink: swallowed exception", exc_info=True)
    _clear_pb_device_worker_state(ws, task, q)
