    ...
    *,
    ...
    while True:
        try:
            ...
            return
        now = time.monotonic()
        queue_wait_ms = (now - frame.received_mono) * 1000.0 if getattr(frame, "received_mono", 0) else -1.0
        gap_ms = (now - last_receive_mono) * 1000.0 if last_receive_mono else -1.0
        last_receive_mono = now
        if camera_pipe is None or camera_image_broker is None:
            _note_camera_frame_dropped(device_id, "no_pipeline", f"pipe={camera_pipe is not None} broker={camera_image_broker is not None}")
            continue
        ...
        schedule_ms = (time.monotonic() - started) * 1000.0
        if schedule_ms > 300.0 or queue_wait_ms > 300.0:
            logger.info(
                "[/asr_chat] camera ingress slow device_id=%s seq=%d queue_wait_ms=%.0f "
                "gap_ms=%.0f schedule_ms=%.0f accepted=%s",
                device_id,
                frame.sequence,
                queue_wait_ms,
                gap_ms,
...
_camera_drop_log_mono: dict[tuple[str, str], float] = {}
_camera_accept_stats: dict[str, list[float]] = {}


def _note_camera_frame_accepted(device_id: Optional[str], nbytes: int) -> None:
    """入库成功：每设备 10s 一行 INFO（帧数 / 最近间隔），与设备端 stat 对账。"""

    key = str(device_id or "")
    now = time.monotonic()
    stats = _camera_accept_stats.setdefault(key, [0.0, 0.0, 0.0])  # [last_log, count, last_frame]
    stats[1] += 1
    gap_ms = (now - stats[2]) * 1000.0 if stats[2] else 0.0
    stats[2] = now
    if now - stats[0] >= 10.0:
        stats[0] = now
        logger.info(
            "[/asr_chat] camera frames accepted device_id=%s total=%d last_gap_ms=%.0f bytes=%d",
            device_id,
            int(stats[1]),
            gap_ms,
            nbytes,
...
def _note_camera_frame_dropped(device_id: Optional[str], reason: str, detail: str) -> None:
    """相机帧在入库前被丢弃：每个 (设备, 原因) 10s 一行 INFO。

    2026-09-07：设备每秒上传 1–2 帧、计数全绿，PC 首页却 8s 内一帧都拿不到——
    丢弃点全是 debug 级，排查只能靠猜。"""

    key = (str(device_id or ""), reason)
    now = time.monotonic()
    last = _camera_drop_log_mono.get(key, 0.0)
    if now - last < 10.0:
        return
    _camera_drop_log_mono[key] = now
    logger.info(
        "[/asr_chat] camera frame dropped device_id=%s reason=%s %s",
        device_id,
        reason,
        detail,
    )
...
async def _schedule_camera_jpeg(
    ...
    *,
    ...
    if generation is None:
        _note_camera_frame_dropped(
            device_id,
            "no_generation",
            f"source={camera_pipe.frame_source} "
            f"pipe_generation={camera_pipe.connection_generation} "
            f"active={current_vision_generation(camera_pipe.device_id)} "
            f"active_source={current_vision_source(camera_pipe.device_id)}",
        )
        return False
    ...
    try:
        ...
        _note_camera_frame_dropped(
            device_id, "invalid_jpeg", f"bytes={len(frame_bytes)} err={exc}"
        )
        return False
    ...
    validate_ms = (time.monotonic() - stage_started) * 1000.0
    _note_camera_frame_accepted(device_id, len(frame_bytes))
    stage_started = time.monotonic()
    ...
    publish_ms = (time.monotonic() - stage_started) * 1000.0
    if validate_ms > 300.0 or publish_ms > 300.0:
        logger.info(
            "[/asr_chat] camera stage slow device_id=%s validate_ms=%.0f publish_ms=%.0f",
            device_id,
            validate_ms,
            publish_ms,
        )
