...
from deskbot_server.application.camera_preview import CameraPreviewLeaseManager, parse_preview_fps
...
async def handle_camera_view(
    ...
    fps = parse_preview_fps(qargs.get("fps"))  # 首页 1 fps、调试页默认 2 fps；设备按所有订阅者的最大值走
    peer = _peer_str(websocket)
    logger.info("[/camera_view] 订阅者接入 peer=%s device_filter=%s fps=%d", peer, url_device, fps)
    ...
    try:
        if leased:
            ...
            await preview_leases.acquire(url_device, fps)
        ...
        try:
            ...
            if leased:
                await preview_leases.release(url_device, fps)
