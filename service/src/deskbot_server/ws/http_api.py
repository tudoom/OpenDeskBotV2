...
from deskbot_server.application.quest_service import state_fingerprint as quest_state_fingerprint
from deskbot_server.application.rtc_instructions import drain as drain_rtc_instructions
from deskbot_server.application.servo_budget import servo_budget
from deskbot_server.application.turn_arbiter import (
    PRIORITY_MANUAL,
    device_turn_arbiter,
)
from deskbot_server.application.user_activity import note as note_user_activity
...
from deskbot_server.llm.prompt_assembly import assemble_rtc_system_prompt
...
def _build_http_request_handler(
    ...
    *,
    ...
    serial_manager_provider: Any | None = None,
):
    ...
    - GET /api/wifi_status?device_id=…：向设备查询 WiFi 状态（经当前会话的控制通道）。
    - POST /api/wifi_provision：JSON ``device_id``（可选 ``ssid``/``password``，缺省
      读取本机 netsh 当前连接的 WiFi），把凭据 + 本机局域网地址 + 链路令牌下发给设备。
    - GET /health：存活探针
    ...
    from deskbot_server.ws.routes import ROUTES, ROUTES_API_KEY, RouteContext, RouteRequest
    ...
    def _peer_from_conn(connection) -> str:
        try:
            ...
            logger.debug("_peer_from_conn: swallowed exception", exc_info=True)
        return "?"
    ...
    # ---- 由 ws/routes 表分发（逐步把 handler 里的分支搬出去；见 ws/routes/__init__.py）----
    _route_ctx = RouteContext(
        registry=registry,
        asr_chat_hub=asr_chat_hub,
        serial_manager_provider=serial_manager_provider,
        rtc_status_provider=rtc_status_provider,
        json_resp=_json_resp,
        cors_headers=_cors_headers,
        no_content=_no_content,
        connection_is_loopback=_connection_is_loopback,
        Response=Response,
        Headers=Headers,
        logger=logger,
    )
    ...
    async def handler(connection, request):
        ...
        if method == "OPTIONS" and path_only.startswith("/api/"):
            return _no_content()
        route = ROUTES.get(path_only)
        if route is not None:
            return await route(
                _route_ctx,
                RouteRequest(
                    path_only=path_only, method=method, qargs=qargs, peer=peer,
                    request=request, connection=connection,
                ),
            )


        if path_only == "/internal/rtc/turns":
            # 语音对话轮次由 Agent worker 回传落库，让“Agent 对话”页的
            # 历史实时包含语音交互。鉴权与 tools 桥完全一致。
            if method != "POST":
            ...
            if not isinstance(body, dict):
                return _json_resp(400, {"ok": False, "error": "invalid_json"})
            user_text = str(body.get("user_text") or "").strip()[:8000]
            assistant_text = str(body.get("assistant_text") or "").strip()[:8000]
            if not user_text and not assistant_text:
                return _json_resp(400, {"ok": False, "error": "empty_turn"})
            if user_text:
                note_user_activity(str(body.get("device_id") or ""))  # 主人侧的话：主动陪伴据此判断「回应了」

            def _persist_voice_turn() -> str:
                from deskbot_server.session_store import (
                    append_turn,
                    ensure_active_session,
                )

                session_id = ensure_active_session()["session_id"]
                append_turn(session_id, user_text, assistant_text)
                return session_id

            try:
                session_id = await asyncio.to_thread(_persist_voice_turn)
            except Exception as exc:
                logger.warning("[rtc][turns] persist failed: %s", exc)
                return _json_resp(500, {"ok": False, "error": "persist_failed"})
            return _json_resp(200, {"ok": True, "session_id": session_id})

        if path_only == "/internal/rtc/instructions":
            # 语音 Agent 轮询：取走 Core 排给它的「现在请你说」指令（主动陪伴 / 提醒）。
            if method != "GET":
                return _json_resp(405, {"ok": False, "error": "method_not_allowed"})
            if not _connection_is_loopback(connection):
                return _json_resp(403, {"ok": False, "error": "loopback_required"})
            ...
            if not expected_token or not supplied_token or not hmac.compare_digest(
                ...
                return _json_resp(401, {"ok": False, "error": "rtc_turn_bridge_unauthorized"})
            dev = str(qargs.get("device_id") or "").strip()
            if not dev:
                return _json_resp(400, {"ok": False, "error": "missing device_id"})
            items = drain_rtc_instructions(dev)
            if items:
                logger.info("[rtc][instructions] drained device_id=%s n=%d", dev, len(items))
            out: dict[str, Any] = {"ok": True, "instructions": items}
            # 顺带让语音 Agent 的任务列表跟上：状态指纹变了就把整份系统提示重发一遍
            known = str(qargs.get("prompt_version") or "").strip()
            try:
                version = await asyncio.to_thread(quest_state_fingerprint)
                if known != version:
                    out["system_prompt"] = await asyncio.to_thread(assemble_rtc_system_prompt)
                    # 装配时可能补建实例 / 搬家，指纹以装配后的状态为准，免得下一轮又发一遍
                    version = await asyncio.to_thread(quest_state_fingerprint)
                out["prompt_version"] = version
            except Exception:  # noqa: BLE001
                logger.debug("[rtc][instructions] prompt refresh failed", exc_info=True)
                    ...
                    if kind == "device_servo" and isinstance(
                        ...
                        expression = await play_web_expression_frames(
                            ...
                            assets=[
                                ...
                                for item in normalize_scene_assets(operation_payload.get("scene_assets"))
                            ],
                    ...
                    if kind in {
                        ...
                        if isinstance(frames, list):
                            arbitrated = await play_web_expression_frames(
                                ...
                                assets=[
                                    base64.b64decode(item)
                                    for item in normalize_scene_assets(operation_payload.get("assets"))
                                ],
        ...
        # ---- 门禁后的路由表分发（必须过 API Key；见 ws/routes/__init__.py）----
        keyed_route = ROUTES_API_KEY.get(path_only)
        if keyed_route is not None:
            return await keyed_route(
                _route_ctx,
                RouteRequest(
                    path_only=path_only, method=method, qargs=qargs, peer=peer,
                    request=request, connection=connection,
                    api_auth=api_auth, device_route_error=_device_route_error,
                ),
            )
        ...
        if path_only == "/api/device_servo":
            ...
            if with_scene_durable:
                ...
                scene_assets_durable = list(scene_entry.get("assets") or [])
            ...
            try:
                ...
                # 运动预算：手动/语音/剧情动作不丢，超额时拉长时长。
                durable_servo, _budget_decision = servo_budget().apply(
                    dev, durable_servo, source="http_device_servo"
                )
                if _budget_decision.stretch > 1:
                    ms = int(ms) * _budget_decision.stretch
            ...
            return await _start_pb_control_operation(
                ...
                operation_payload={
                    ...
                    "scene_assets": scene_assets_durable,
                },
                requested_operation_id=str(
                    (
        ...
        if path_only == "/api/device_face_play":
            ...
            def _build_face_sequences(request_id: str):
                durable_pairs = design_frames_to_pb_chain(
                    ent.get("frames") or [],
                    assets=decode_scene_assets(ent),
            ...
            return await _start_pb_control_operation(
                ...
                operation_payload={
                    ...
                    # 位图（卡通）场景的 JPEG 附件必须随 operation 一起走：运行时按 payload 的
                    # assets 补发二进制，缺了就是设备端 "image asset=0 invalid"（2026-09-07 真机复现）。
                    "assets": list(ent.get("assets") or []),
                },
        ...
        if path_only == "/api/device_tts":
            ...
            if isinstance(payload, dict):
                ...
                if raw_moves is not None:
                    if not isinstance(raw_moves, list) or not all(
                        isinstance(m, dict) for m in raw_moves
                    ):
                        return _json_resp(
                            400,
                            {
                                "ok": False,
                                "error": "moves must be an array of objects",
                                "t": time.time(),
                            },
                        )
                    moves_q = [dict(m) for m in raw_moves]
            if not dev:
                return _json_resp(
                    400,
                    {"ok": False, "error": "missing device_id", "t": time.time()},
        ...
        if path_only == "/api/device_pb_anim":
            ...
            if isinstance(anim, dict):
                ...
                # 卡通（位图）表情：随请求带 JPEG 资产；缺失时 image 图元会在设备端被丢弃
                try:
                    pb_anim_assets = [
                        base64.b64decode(item)
                        for item in normalize_scene_assets(body.get("assets"))
                    ]
                except ValueError as exc:
                    return _json_resp(
                        400, {"ok": False, "error": f"assets 无效：{exc}", "t": time.time()}
                    )
            ...
            return await _start_pb_control_operation(
                ...
                operation_payload={
                    ...
                    "assets": list(normalize_scene_assets(body.get("assets"))),
