    ...
    try:
        while not handler.done():
            ...
            try:
                ...
                logger.debug("run_with_debug_subscriber_auth_watchdog: swallowed exception", exc_info=True)
...
async def enforce_turn_usage(
    ...
    *,
    ...
    try:
        ...
        logger.debug("enforce_turn_usage: swallowed exception", exc_info=True)
    try:
...
async def settle_turn_usage(
    ...
    *,
    ...
    try:
        ...
        logger.debug("settle_turn_usage: swallowed exception", exc_info=True)
    try:
        await websocket.close(code=close_code, reason=error)
    except Exception:
        logger.debug("settle_turn_usage: swallowed exception", exc_info=True)
