...
_g2p: Any | bool | None = None
_g2p_warning_logged = False

# Keep a tiny, deterministic viseme lexicon for common words. This is not a
# replacement for g2p-en; it keeps lip sync usable when optional NLTK data is
# unavailable (for example, during first boot without network access).
_ENGLISH_VISEME_FALLBACKS: dict[str, tuple[str, ...]] = {
    "hello": ("h", "a", "l", "o"),
    "world": ("w", "e", "l", "d"),
    "hi": ("h", "ai"),
}
...
def _get_g2p():
    global _g2p
    if _g2p is False:
        raise RuntimeError("g2p-en NLTK data is not installed")
    if _g2p is None:
        # Importing g2p_en tries to download missing NLTK assets. Server
        # request handling must never trigger an implicit network download, so
        # only import it after all locked CI/runtime assets are present.
        try:
            import nltk
            ...
            _g2p = False
            raise RuntimeError("g2p-en NLTK data is not installed") from None
...
def _fallback_english_phoneme_units(word: str) -> list[tuple[str, float]]:
    normalized = re.sub(r"[^a-z]", "", str(word or "").lower())
    if not normalized:
        return [("_", 1.0)]
    known = _ENGLISH_VISEME_FALLBACKS.get(normalized)
    if known:
        return [
            (phone, 0.65 if phone in {"a", "e", "i", "o", "u", "ai"} else 0.35)
            for phone in known
        ]
    ...
    while cursor < len(normalized):
        pair = normalized[cursor : cursor + 2]
        if pair in digraphs:
            phone = digraphs[pair]
            cursor += 2
        else:
            phone = letters.get(normalized[cursor], "_")
            cursor += 1
        if phone != "_" and (not phones or phones[-1] != phone):
            phones.append(phone)
    if not phones:
        return [("_", 1.0)]
    return [
        (phone, 0.65 if phone in {"a", "e", "i", "o", "u", "ai", "ou"} else 0.35)
        for phone in phones
    ]
...
def _english_phoneme_units(word: str) -> list[tuple[str, float]]:
    global _g2p_warning_logged

    phones: list[tuple[str, float]] = []
    try:
        raw_phones = _get_g2p()(str(word or "").lower())
    except Exception as exc:
        if not _g2p_warning_logged:
            _g2p_warning_logged = True
            logger.warning(
                "[TTS/doubao_words] G2P unavailable; using deterministic "
                "offline viseme fallback content=%s err=%s",
                safe_log_content(word),
                exc,
            )
        raw_phones = []
    ...
    if phones:
        return phones
    return _fallback_english_phoneme_units(word)
