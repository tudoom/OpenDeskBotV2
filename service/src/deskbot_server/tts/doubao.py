...
# 出厂默认音色：佩奇猪 2.0（产品定调）。
DEFAULT_SPEAKER = "zh_female_peiqi_uranus_bigtts"
...
class DoubaoTtsConfig:
    ...
    # 声音复刻要用的音色 ID，由用户在火山控制台申请后填写（形如 S_xxxx）。
    # 带内置密钥的分发版在种子 .env 里预置一个，普通版留空由用户自己填。
    voice_clone_speaker_id: str = ""
...
def load_doubao_tts_config() -> DoubaoTtsConfig:
    ...
    return DoubaoTtsConfig(
        ...
        voice_clone_speaker_id=(
            os.environ.get("DOUBAO_TTS_VOICE_CLONE_SPEAKER_ID") or ""
        ).strip(),
    ...
    if isinstance(exc, InvalidStatus):
        ...
        if status == 401:
            hint = (
                ...
                "使用 DOUBAO_TTS_API_KEY，不要使用 LLM_API_KEY）"
            )
...
class DoubaoTtsConnection:
    ...
    async def _ensure_ready(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug("_ensure_ready: swallowed exception", exc_info=True)
            raise
        self._ws = ws
        self._ready = True
        logger.info(
    ...
    async def _reset(self) -> None:
        ...
        try:
            ...
            logger.debug("_reset: swallowed exception", exc_info=True)
        try:
