...
# 倾听看门狗：VAD「开口」之后这么久还没有确认说完，就当作误报（噪声 / 旁人），倾听脸回待机。
USER_SPEECH_STALE_SEC = 8.0
...
class _UsbRtcBinding:
    ...
    user_speech_started_at: float = 0.0
    # 倾听看门狗：VAD 报了开口却一直没有 confirmed_end（环境噪声 / 旁人说话），倾听脸会挂到天荒地老，
    # 顺带挡住待机张望和主动陪伴的安静计时（2026-09-11 12:47 实际发生）。到时没说完就当误报回待机。
    user_speech_watchdog: asyncio.Task | None = None
    ...
    def _cancel_user_speech_watchdog(self) -> None:
        task = self.user_speech_watchdog
        self.user_speech_watchdog = None
        if task is not None and not task.done():
            task.cancel()

    def _arm_user_speech_watchdog(self, started_at: float) -> None:
        self._cancel_user_speech_watchdog()
        self.user_speech_watchdog = asyncio.create_task(
            self._expire_stale_user_speech(started_at),
            name=f"deskbot-rtc-speech-watchdog:{self.rtc_session.device_id}",
        )

    async def _expire_stale_user_speech(self, started_at: float) -> None:
        try:
            await asyncio.sleep(USER_SPEECH_STALE_SEC)
        except asyncio.CancelledError:
            raise
        if not self.user_speech_active or self.user_speech_started_at != started_at:
            return
        self.user_speech_active = False
        self.user_speech_watchdog = None
        logger.info(
            "[rtc] user speech never confirmed within %.0fs, resolving listening back to idle device_id=%s",
            USER_SPEECH_STALE_SEC,
            self.rtc_session.device_id,
        )
        if self.expression_runtime is not None and self.agent_state in {"", "idle", "listening"}:
            try:
                await self.expression_runtime.transition("idle", reason="user_speech_stale")
            except Exception:  # noqa: BLE001 - 表情回不去也不能影响别的
                logger.debug("[rtc] stale-listening idle transition failed", exc_info=True)
...
async def bind_usb_device(
    ...
    *,
    ...
    live_behavior_service().start_device(device_id)
    gateway = _gateway
    if gateway is None:
        ...
        try:
            ...
            async def _playback_control(
                ...
                if event == "barge_in":
                    ...
                    active.user_speech_started_at = time.monotonic()
                    ...
                    active.user_speech_active = True
                    active.user_speech_started_at = time.monotonic()
                    active._arm_user_speech_watchdog(active.user_speech_started_at)
                    if active.expression_runtime is not None:
                        preempt = getattr(
                            active.expression_runtime,
                            "preempt_lease",
                    ...
                    active._cancel_user_speech_watchdog()
                    if active.expression_runtime is not None:
                        await active.expression_runtime.transition(
                            "thinking",
                    ...
                    if active.note_agent_state(state):
                        ...
                        # 打断后被中止的那一轮会紧接着报 idle：用户还在说话，屏幕却从
                        # 倾听翻回待机（卡通脸上一眼可见，矢量脸两者相近没人注意）。
                        # 用户刚开口几秒内到来的 idle 一律忽略，倾听脸保持到说完。
                        speech_fresh = (
                            active.user_speech_active
                            and time.monotonic() - float(active.user_speech_started_at or 0.0) < 4.0
                        )
                        if state in {"thinking", "speaking"} or (state == "idle" and not speech_fresh):
                            active.user_speech_active = False
                            active._cancel_user_speech_watchdog()
                        visible_state = state
                        ...
                        if state == "initializing":
                            visible_state = ""
                        elif state == "idle" and speech_fresh:
                            visible_state = ""
...
async def unbind_usb_device(device_id: str, device_session: object) -> None:
    ...
    await live_behavior_service().stop_device(device_id)
