...
try:
    ...
    def _deskbot_silero_vad_load(*args, **kwargs):
        ...
        # 2026-09-09：0.30s 会把"叫我…小朋友"在"小"处截断；报名字/想词的停顿常到 0.5s。
        kwargs.setdefault("min_silence_duration", 0.55)
    ...
    print(
        ...
        "deactivation=0.10 silence=0.55s",
        file=sys.stderr,
        flush=True,
    )
...
try:
    from deskbot_server.rtc_llm_adapter import install_lampgo_llm_adapter
    llm_adapter_installed = install_lampgo_llm_adapter()
    print(
        "[deskbot-rtc] Ark Responses LLM adapter installed=%s pid=%d"
        % (llm_adapter_installed, os.getpid()),
        file=sys.stderr,
        flush=True,
    )
except Exception as exc:
    # 非致命：退回 Chat Completions 链路，联网仍可走 Core websearch 工具。
    print(
        "[deskbot-rtc] LLM adapter setup failed: %r" % (exc,),
        file=sys.stderr,
        flush=True,
    )
...
try:
    ...
    def _deskbot_agent_session_init(self, *args, **kwargs):
        ...
        try:
            ...
            print("[deskbot-rtc] turn detector unavailable: %r" % (exc,), file=sys.stderr, flush=True)
...
# 语音提示词装配已上移到 llm/prompt_assembly；此处保留名字供既有调用与测试。
from deskbot_server.llm.prompt_assembly import rtc_home_control_prompt  # noqa: E402,F401
...
class RtcAgentSdkManager:
    ...
    def _can_start(self) -> bool:
        ...
        llm_key = _first_env("LLM_API_KEY", "OPENAI_API_KEY")
    ...
    def _rtc_system_prompt(self) -> str:
        ...
        return assemble_rtc_system_prompt()
            ...
            "defaults": {
                ...
                "tts": {
                    ...
                    "options": {
                        ...
                        or "zh_female_peiqi_uranus_bigtts",
                        "sample_rate": int(
    ...
    def _find_port_listener_pid(self) -> int | None:
        ...
        try:
            ...
            # macOS restricts net_connections() for some processes.
            # Fall back to lsof which does not require elevated permissions.
            return self._find_port_listener_pid_lsof()
    ...
    @staticmethod
    def _find_port_listener_pid_lsof() -> int | None:
        import subprocess

        try:
            result = subprocess.run(
                ["lsof", "-ti", f"tcp:{AGENT_SDK_PORT}", "-sTCP:LISTEN"],
                capture_output=True,
                text=True,
                timeout=5,
            )
