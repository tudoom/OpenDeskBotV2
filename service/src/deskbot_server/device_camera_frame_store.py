...
# 固件 0.0.37 起 camera_once 走控制帧快速路径，一张快照通常 0.5~2s 内回来；主等待给足 2.5s，
# 避免动不动就切到 2fps 连续取帧——连续取帧会撞上 LCD_CAM 驱动的 ~4s 等待，把设备主循环拖停、
# 链路 ACK 超时重连（2026-09-07 真机：usb_poll_max_gap_ms=4214）。
_ONE_SHOT_PRIMARY_WAIT_S = 2.5
...
def update_device_camera_frame(
    ...
    *,
    ...
    try:
        from deskbot_server.device_camera_health import observe_frame

        observe_frame(device_id)
    except Exception:  # noqa: BLE001 —— 健康度只是遥测，不能影响帧入库
        logger.debug("update_device_camera_frame: swallowed exception", exc_info=True)
    dev = str(device_id or "").strip()
    if not dev:
        return False
...
def wait_for_device_camera_frame(
    ...
    *,
    ...
    with _frame_ready:
        row = _by_device.get(dev)
    logger.info(
        "[capture_camera] wait timed out device_id=%s timeout=%.1fs after_ts_age=%.1fs "
        "max_age=%.1fs cached=%s",
        dev,
        float(timeout),
        time.time() - float(after_ts) if after_ts > 0 else -1.0,
        float(max_age_s),
        (
            f"age={time.time() - float(row.get('captured_at') or 0):.1f}s "
            f"gen={row.get('generation')} src={row.get('source')}"
            if row
            else "none"
        ),
    )
