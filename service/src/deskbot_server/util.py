...
def _normalize_incoming_pb_ack(data: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    # ACKs from older firmware only confirm that the chunk entered the device
    # playback queue.  They must never be upgraded to "played".
    out["phase"] = (
        phase
        if phase in ("accepted", "played", "failed", "cancelled")
        else "accepted"
    )
    error = data.get("error")
    if isinstance(error, str) and error.strip():
        out["error"] = error.strip()[:512]
    r = data.get("req")
    ...
    try:
        ...
        out["audio_buf_ms"] = 0
    display_crc32 = str(data.get("display_crc32") or "").strip().lower()
    if (
        out["phase"] == "played"
        and len(display_crc32) == 8
        and all(char in "0123456789abcdef" for char in display_crc32)
    ):
...
def _split_path(raw_path: str) -> tuple:
    """把 `/camera_view?role=subscriber` 拆成 (path, query)。"""
    if not raw_path:
        return "", ""
    if "?" in raw_path:
...
def _peer_str(websocket) -> str:
    ...
    try:
        ...
        logger.debug("_peer_str: swallowed exception", exc_info=True)
    return "?"
