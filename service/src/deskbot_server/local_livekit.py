...
_BINARY_NAME = "livekit-server.exe" if os.name == "nt" else "livekit-server"
_DEFAULT_BINARY = DATA_DIR / "local" / "livekit" / _BINARY_NAME
...
class LocalLiveKitServerManager:
    ...
    def _resolve_binary(self) -> Path | None:
        ...
        if stable_bin_dir:
            candidates.append(Path(stable_bin_dir) / _BINARY_NAME)
        configured = str(
    ...
    async def _start_once(self) -> bool:
        try:
            ...
            if binary is None:
                self._last_error = (
                    "local livekit-server binary is missing; "
                    "install via Homebrew (brew install livekit/livekit/livekit) "
                    "or run tools/install_livekit_windows.ps1 on Windows"
                )
                return False
            port = self._server_port()
