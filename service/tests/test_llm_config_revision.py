...
def test_partial_update_preserves_missing_and_masked_fields(llm_config_env):
    ...
    assert "ARK_API_KEY=secret-original" in env_file.read_text(
        encoding="utf-8"
    ).splitlines()
...
def test_setup_get_patch_round_trip_is_lossless_and_pending(
    ...
    env_file, _state_file = llm_config_env
    _write_initial_env(env_file)
    # ARK_API_KEY 已不再充当 LLM 凭证：Web 保存守卫要求真正的 LLM_API_KEY。
    env_file.write_text(
        env_file.read_text(encoding="utf-8") + "LLM_API_KEY=secret-llm" + chr(10),
        encoding="utf-8",
    )
    load_dotenv(force_reload=True)
...
def test_removed_ark_field_is_ignored_by_save(llm_config_env):
    """ark_api_key 字段已废弃：保存被忽略、不写盘、不动 revision。"""
    from deskbot_server.llm.config_state import get_llm_config_status
    from deskbot_server.llm.env_store import save_llm_env
    ...
    persisted = env_file.read_text(encoding="utf-8").splitlines()
    assert "ARK_API_KEY=ark-new" not in persisted
    assert "ARK_API_KEY=secret-original" in persisted
