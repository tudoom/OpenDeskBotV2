...
def test_wifi_page_merged_into_device_connection(local_client):
    client, _app = local_client
    # 旧 /wifi 永久并入「设备连接」，保留跳转以兼容书签。
    ...
    assert "设备连接" in html
    assert "一键同步" in html
    assert "/proxy/deskbot/api/wifi_provision" in html
    assert "/proxy/deskbot/api/wifi_status" in html
    assert "transportLabel" in html
    # 机器人按编号找 Core：页面展示本机 Core 身份与设备记住的 Core。
    assert "本机 Core" in html
    assert "coreBindingLabel" in html
    assert "记住的 Core" in html
    # 片内温度与告警：连接页设备卡、首页横幅、顶栏全局提示（固件 ≥0.0.52 上报）
    assert "芯片温度" in html and "d.thermal.message" in html
    home = client.get("/home").get_data(as_text=True)
    assert "设备温度过高" in home
    js = (Path(__file__).resolve().parents[1] / "src/deskbot_server/web/static/device_selector.js").read_text(encoding="utf-8")
    assert "renderThermalNotice" in js and "globalDeviceNotice" in js
    # 侧边栏只剩单一入口，不再有独立 WiFi 配网页链接。
    assert 'data-label="WiFi 配网"' not in html
    assert 'data-label="USB 设备"' not in html
    assert 'data-label="设备连接"' in html
    # 「固件更新」是无详情页的状态 tab：按钮 + 徽标 + 状态轮询，无路由。
    assert 'id="fwUpdateTab"' in html
    assert 'id="fwUpdateBadge"' in html
    assert "/proxy/deskbot/api/firmware_manifest" in html
    assert "/proxy/deskbot/api/firmware_update" in html
    assert 'href="/firmware"' not in html
...
def test_expression_editor_keeps_local_library_and_device_preview_separate(local_client):
    ...
    # 「我的表情」更新按钮已撤：编辑后经「保存到库」另存。
    assert "updateUserExpression" not in html
    ...
    # 执行日志随首页改版搬去 /home，后又扩展为「Agent 全面日志」。
    # 2026-09-03 按产品要求去掉了「已确认执行的完整表情 / 来源·原因·RTC 目标」
    # 两行细节，只留对话状态、RTC 会话与麦克风告警；这里钉住剩下的这三项。
    home_html = client.get("/home").get_data(as_text=True)
    assert "Agent 全面日志" in home_html
    assert "设备已确认执行的完整表情" not in home_html
    assert "rtcStateLabel" in home_html
    assert "agentStateLabel" in home_html
    assert "data-page-microphone-health-alert" in home_html
    # 首页相机 = 订阅 /camera_view 实时流（1 fps），只在首页且页面可见时接入；不再轮询抓拍
    assert "&fps='+HOME_CAMERA_FPS" in home_html and "const HOME_CAMERA_FPS = 1;" in home_html
    assert "new WebSocket(" in home_html and "visibilitychange" in home_html and "pagehide" in home_html
    assert "camera_snapshot?" not in home_html and "&warm=1" not in home_html
    assert "owner_user_id" not in html
...
def test_expression_editor_uses_one_revisioned_transaction_contract(local_client):
    ...
    # 口型页读同一快照的 mouth_groups，恢复出厂也走同一个事务契约。
    assert "mouth_groups" in html
    assert "phonemes:{factory_reset:true}" in html
...
def test_expression_editor_honors_origin_and_preserves_scene_frames(local_client):
    ...
    assert '@click="updateUserExpression(s)"' not in html
...
def test_expression_editor_rebases_dirty_mapping_and_guards_mutations(local_client):
    ...
    # 情绪→表情映射编辑 UI 已撤（运行时映射仍在，默认表情写 idle 键也在）。
    assert "保存映射" not in html
    assert "markMapChanged" not in html
    # AI 生成合并后由单个生成按钮把关，生成期间不可重复提交。
    assert ':disabled="generatingImageExpression || generatingTextExpression"' in html
...
def test_advanced_llm_card_offers_switchable_providers(local_client):
    ...
    assert "provider:'deepseek'," in html and "this.setup.provider='deepseek';" in html
    assert "provider:'doubao'," not in html and "this.setup.provider='doubao';" not in html
...
def test_voice_page_hides_multilingual_and_lists_every_voice(local_client):
    """2C 声音页：不展示多语种英文音色；优选音色一次全部列出，不再收起。"""
    client, _app = local_client
    from deskbot_server.tts.speakers import list_doubao_tts_consumer_speaker_presets

    rows = list_doubao_tts_consumer_speaker_presets()
    assert rows, "2C 音色清单不应为空"
    assert not [r for r in rows if r.get("scene") == "多语种"]
    # 目录数据本身保持完整：其它入口仍能拿到多语种音色
    from deskbot_server.tts.speakers import find_doubao_tts_speaker_preset

    assert find_doubao_tts_speaker_preset("en_female_dacey_uranus_bigtts") is not None
