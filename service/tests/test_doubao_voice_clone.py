...
def test_status_query_uses_the_custom_speaker_convention(monkeypatch):
    ...
    result = voice_clone.get_doubao_voice_clone_status(cfg, "demo_voice")
    assert seen["payload"]["speaker_id"] == "custom_speaker_id"
    assert seen["payload"]["custom_speaker_id"] == "demo_voice"
    assert result.status == 4
    assert result.model_type == 5
    assert result.ready is True
...
def test_clone_requires_user_supplied_speaker_id():
    """音色 ID 由用户填写：留空必须报错，而不是由服务端生成一个代号。"""
    import pytest
    ...
    # 非官方前缀：训练直填会被判资源不匹配，查询又走自定义路径——必须在入口拦下
    with pytest.raises(ValueError, match="官方"):
        normalize_speaker_id("my_voice")
    with pytest.raises(ValueError):
        normalize_speaker_id("S_" + "x" * 100)
