...
def test_firmware_builtin_face_geometry_lockstep_with_spec():
    ...
    nose_src = face.split("StoredPrim nose{};", 1)[1].split(
        "s_pb_curr_nose.prims", 1
    )[0]
    mouth_src = face.split("StoredPrim mouth{};", 1)[1].split(
        "s_pb_curr_mouth.prims", 1
    )[0]
    slit_src = face.split("StoredPrim slit{};", 1)[1].split(
        "s_pb_curr_mouth.prims", 1
    )[0]
    ...
    nose = _fields(nose_src, "nose")
    mouth = _fields(mouth_src, "mouth")
    slit = _fields(slit_src, "slit")

    assert "PbShape::EllipseFill" in eye_left_src
    assert "PbShape::Circle;" in nose_src
    assert "PbShape::RoundRect;" in mouth_src
    assert "PbShape::RoundRect;" in slit_src
    assert "kBuiltinFaceColor" in eye_left_src
    assert "kBuiltinNoseColor" in nose_src
    assert "kBuiltinFaceColor" in mouth_src
    assert "DESKBOT_DISPLAY_COLOR_BLACK" in slit_src
    assert "fillScreen(DESKBOT_DISPLAY_COLOR_BLACK)" in DISPLAY

    # 固件颜色常量与 spec 逐一相等：眼/嘴青、鼻黄、嘴缝黑，背景刷黑。
    face_color = int(
        re.search(r"kBuiltinFaceColor = 0x([0-9A-Fa-f]+);", DISPLAY).group(1), 16
    )
    nose_color = int(
        re.search(r"kBuiltinNoseColor = 0x([0-9A-Fa-f]+);", DISPLAY).group(1), 16
    )
    assert face_color == spec.BUILTIN_FACE_RGB565
    assert nose_color == spec.BUILTIN_NOSE_RGB565
    panel = (ROOT / "hardware" / "firmware" / "display_panel.h").read_text(
        encoding="utf-8"
    )
    assert "#define DESKBOT_DISPLAY_COLOR_BLACK ST77XX_BLACK" in panel
    ...
    spec_nose = spec.BUILTIN_FACE_ELEMENTS["nose"][0]
    spec_mouth, spec_slit = spec.BUILTIN_FACE_ELEMENTS["mouth"]

    # StoredPrim 的 w/h 对应 wire ellipse 的 rw/rh；r 对应 circle r / round_rect radius。
    ...
    assert spec_eye_l["c"] == spec.BUILTIN_FACE_RGB565
    assert spec_eye_r["shape"] == "ellipse_fill"
    assert (spec_eye_r["x"], spec_eye_r["y"]) == (
        eye_r_override["x"],
        eye_r_override["y"],
    )
    assert (spec_eye_r["rw"], spec_eye_r["rh"]) == (eye_l["w"], eye_l["h"])
    assert spec_eye_r["c"] == spec.BUILTIN_FACE_RGB565

    assert spec_nose["shape"] == "circle"
    assert (spec_nose["x"], spec_nose["y"], spec_nose["r"]) == (
        nose["x"],
        nose["y"],
        nose["r"],
    )
    assert spec_nose["sw"] == nose["stroke_width"]
    assert spec_nose["c"] == spec.BUILTIN_NOSE_RGB565

    assert spec_mouth["shape"] == "round_rect"
    ...
    assert spec_mouth["c"] == spec.BUILTIN_FACE_RGB565

    assert spec_slit["shape"] == "round_rect"
    assert (spec_slit["x"], spec_slit["y"]) == (slit["x"], slit["y"])
    assert (spec_slit["w"], spec_slit["h"]) == (slit["w"], slit["h"])
    assert spec_slit["radius"] == slit["r"]
    assert spec_slit["sw"] == slit["stroke_width"]
    assert spec_slit["c"] == 0x0000
...
def test_builtin_face_matches_template_idle_frame():
    """待机脸 = 模板 ``idle`` 第 0 帧：未连接 PC 与连接后默认表情同像素。"""
    import json

    template = json.loads(
        (ROOT / "service" / "data" / "global" / "deskbot-face.json").read_text(
            encoding="utf-8"
        )
    )
    idle = next(s for s in template["emotions"] if s["name"] == "idle")
    frame = idle["frames"][0]["elements"]

    def _norm(prims: list[dict]) -> list[dict]:
        keep = ("shape", "x", "y", "w", "h", "r", "rw", "rh", "radius", "c")
        return [{k: p[k] for k in keep if k in p} for p in prims]

    for layer in ("eye_l", "eye_r", "nose", "mouth"):
        assert _norm(frame.get(layer, [])) == _norm(
            spec.BUILTIN_FACE_ELEMENTS[layer]
        ), layer
