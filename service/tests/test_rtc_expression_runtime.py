...
def test_same_state_is_resent_when_its_mapping_changed():
    """用户换了一套卡通表情后，第一次说话不能被当成"重复生命周期"合并掉：
    屏幕上还是旧套图的说话脸、新映射指向别的场景时，要立即重发（2026-09-07 真机复现）。"""
    from deskbot_server.application.expression_runtime import (
    ...
    def _catalog_b():
        return build_expression_catalog(
            {
                "mappings": {"idle": "idle", "speaking": "sleep"},
                "emotions": [_scene("idle", 1), _scene("happy", 4, aliases=("talk_face",)), _scene("sleep", 5)],
            }
        )

    async def _run():
        # 第 2 次写入阻塞：此时"说话"仍是活动请求，屏幕上是旧套图（happy）
        session = _OrderedUsbSession(blocked_writes=(2,))
        runtime = RtcExpressionRuntime("deskbot_test", session, catalog_loader=_catalog)
        first = await runtime.transition("speaking")
        assert first.resolved == "happy" and first.status == "played"

        runtime._catalog_loader = _catalog_b  # 换套图：speaking → sleep
        second = asyncio.create_task(runtime.transition("speaking"))
        await session.write_events.setdefault(2, asyncio.Event()).wait()
        third = asyncio.create_task(runtime.transition("speaking"))
        await asyncio.sleep(0)
        session.write_releases.setdefault(2, asyncio.Event()).set()
        second_result, third_result = await asyncio.gather(second, third)
        # 新映射的场景必须真的发出去，而且不是被"重复生命周期"合并掉的
        assert _marker(session.messages[-1]) == 5
        assert third_result.status != "coalesced" or second_result.resolved == "sleep"

    asyncio.run(_run())


def test_stale_user_speech_resolves_listening_back_to_idle(monkeypatch):
    """2026-09-11 12:47：VAD 报了「开口」却一直没有 confirmed_end（噪声 / 旁人），倾听脸挂了几分钟，
    张望和主动陪伴的安静计时全被挡住。看门狗：USER_SPEECH_STALE_SEC 内没说完就当误报回待机；
    真的说完（confirmed_end）或模型开始思考 / 说话则看门狗作废。"""
    import asyncio
    from types import SimpleNamespace
    ...
    class _ExpressionRuntime:
        ...
        async def transition(self, state, *, reason):
            expression_events.append((state, reason))
    ...
    monkeypatch.setattr(rtc_runtime, "USER_SPEECH_STALE_SEC", 0.05)
    ...
    async def _run():
        ...
        rtc_runtime.install_rtc_gateway(gateway)
        device = _Device()
        await rtc_runtime.bind_usb_device("deskbot_test", device)
        await gateway.control("agent_state", {"state": "listening"})
        # 误报：开口后没有任何后续 → 看门狗把倾听脸翻回待机
        await gateway.control("user_speech_start", {})
        assert expression_events[-1] == ("listening", "user_speech_start")
        await asyncio.sleep(0.15)
        assert expression_events[-1] == ("idle", "user_speech_stale")
        # 真说话：开口 → 确认说完 → 思考；看门狗作废，不会再把脸翻回去
        await gateway.control("user_speech_start", {})
        await gateway.control("user_speech_confirmed_end", {})
        n = len(expression_events)
        await asyncio.sleep(0.15)
        assert expression_events[n - 1] == ("thinking", "user_speech_confirmed_end") and len(expression_events) == n
        # 开口后模型直接开始说话（打断确认走别的包）：同样作废
        await gateway.control("user_speech_start", {})
        await gateway.control("agent_state", {"state": "speaking"})
        n = len(expression_events)
        await asyncio.sleep(0.15)
        assert len(expression_events) == n and ("idle", "user_speech_stale") not in expression_events[n - 2:]
        await gateway.control("connection_closed", {})
        await rtc_runtime.unbind_usb_device("deskbot_test", device)
