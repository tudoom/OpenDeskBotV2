...
def test_camera_snapshot_route_accepts_a_recent_cached_frame():
    """首页切页回来要能立刻出画：max_age>0 时走缓存帧，不强制重新抓拍。

    每次切页都是整页重载，若一律 require_fresh，OV3660 换节拍后首帧要 4–6s，
    用户每回首页都从黑屏等起。
    """
    ...
    route = (
        ...
        / "src/deskbot_server/ws/routes/camera.py"
    ).read_text(encoding="utf-8")
    assert 'qargs.get("max_age")' in route
    # 设备帧缓存 TTL 只有 5s（服务视觉新鲜度语义，不能为 UI 拉长），
    # 所以预览用的"上一帧"单独缓存在路由里
    assert "_preview_get(dev, max_age)" in route
    assert "_preview_put(dev" in route
    # 上限保护：别让调用方要一张任意老的帧
    assert "min(max(max_age, 0.0), 300.0)" in route

    home = (
        Path(__file__).resolve().parents[1]
        / "src/deskbot_server/web/templates/app2c/home.html"
    ).read_text(encoding="utf-8")
    # 首页不再轮询抓拍：订阅 /camera_view 实时流，1 fps，切到后台就断开
    assert "camera_snapshot?" not in home and "&warm=1" not in home
    assert "&fps='+HOME_CAMERA_FPS" in home and "HOME_CAMERA_HIDE_GRACE_MS" in home
