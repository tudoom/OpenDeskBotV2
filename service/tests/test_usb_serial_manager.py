...
def test_macos_open_does_not_touch_dtr_to_avoid_resetting_the_robot():
    """2026-09-11：macOS 内核 open() 时拉高 DTR/RTS，pyserial 再把 DTR 拉低（RTS 仍高）= ESP32-S3 USB-JTAG 的复位序列，
    Core 每次重启都把机器人重启一遍，开机竞速里 WiFi 先就绪就绑到 WiFi。macOS 用 dsrdtr=True 让 pyserial 不碰 DTR。"""
    from deskbot_server.infrastructure.serial.manager import pyserial_open_kwargs

    assert pyserial_open_kwargs("darwin") == {"dsrdtr": True}
    assert pyserial_open_kwargs("win32") == {} and pyserial_open_kwargs("linux") == {}
