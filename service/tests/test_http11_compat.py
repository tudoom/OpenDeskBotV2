...
def test_protocol_debug_logging_redacts_credentials():
    assert (
        _debug_request_path("/asr_chat?device_id=d1&api_key=odk-secret")
        == "/asr_chat"
    )
    assert _debug_header_value("X-API-Key", "odk-secret") == "<redacted>"
    assert (
        _debug_header_value(
            "Sec-WebSocket-Protocol",
            "deskbot.debug.auth.secret-token",
        )
        == "<redacted>"
    )
    assert _debug_header_value("Content-Type", "application/json") == "application/json"
