...
def _mouth_frame(width: int) -> dict:
    return {
        "ms": 200,
        "elements": {
            "eye_l": [],
            ...
            "extra": [],
            "mouth": [
                {"shape": "rect", "x": 10, "y": 10, "w": width, "h": 4, "c": 65535}
            ],
        },
    }


def test_factory_reset_restores_the_template_mouth_shapes(face_root, monkeypatch):
    """恢复出厂口型 = 用全局模板的 phonemes 整段覆盖，走同一事务契约。"""
    template_path = face_root / "global" / "deskbot-face.json"
    doc = json.loads(template_path.read_text(encoding="utf-8"))
    doc["phonemes"] = [{"name": "a", "title": "啊", "frames": [_mouth_frame(20)]}]
    ...
    state = get_face_expression_state()
    factory_groups = state["mouth_groups"]
    assert factory_groups, "快照必须带口型组，口型页才有数据可显示"

    edited = apply_face_expression_transaction(
        {
            "expected_revision": state["revision"],
            "phonemes": {
                "upsert": [{"name": "a", "title": "啊", "frames": [_mouth_frame(4)]}]
            },
        }
    )
    assert edited["mouth_groups"] != factory_groups

    restored = apply_face_expression_transaction(
        {
            "expected_revision": edited["revision"],
            "phonemes": {"factory_reset": True},
        }
    )
    assert restored["mouth_groups"] == factory_groups
    assert restored["revision"] == edited["revision"] + 1


def test_set_mouth_replaces_only_the_mouth_layer_of_matching_phonemes(face_root):
    """口型页按组编辑：只换命中音素每一帧的 mouth，眼睛等其它层原样保留。"""
    template_path = face_root / "global" / "deskbot-face.json"
    doc = json.loads(template_path.read_text(encoding="utf-8"))
    eye = [{"shape": "pixel", "x": 7, "y": 8, "c": 65535}]
    frame_a = _mouth_frame(20)
    frame_a["elements"]["eye_l"] = eye
    doc["phonemes"] = [
        {"name": "a", "title": "啊", "alias": ["ang"], "frames": [frame_a]},
        {"name": "o", "title": "哦", "frames": [_mouth_frame(21)]},
    ]
    template_path.write_text(json.dumps(doc, ensure_ascii=False), encoding="utf-8")
    clear_face_design_cache()

    state = get_face_expression_state()
    new_mouth = [{"shape": "round_rect", "x": 100, "y": 150, "w": 80, "h": 20, "radius": 6, "c": 2047}]
    saved = apply_face_expression_transaction(
        {
            "expected_revision": state["revision"],
            "phonemes": {"set_mouth": [{"states": ["ang"], "elements": new_mouth}]},
        }
    )
    persisted = load_face_design_file()
    by_name = {row["name"]: row for row in persisted["phonemes"]}
    assert by_name["a"]["frames"][0]["elements"]["mouth"] == new_mouth
    assert by_name["a"]["frames"][0]["elements"]["eye_l"] == eye
    assert by_name["o"]["frames"][0]["elements"]["mouth"] == _mouth_frame(21)["elements"]["mouth"]
        ...
        apply_face_expression_transaction(
            {
                ...
                "phonemes": {"set_mouth": [{"states": ["zzz"], "elements": new_mouth}]},
            }
        )
