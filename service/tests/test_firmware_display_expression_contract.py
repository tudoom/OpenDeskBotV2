...
def test_initialization_shows_standby_screen_and_builtin_only_on_failure():
    ...
    # 内建脸 = 模板 idle 第 0 帧 + 黄鼻子：椭圆眼、圆鼻、圆角矩形填充嘴（含嘴缝）。
    assert "PbShape::EllipseFill" in face
    assert "PbShape::Circle;" in face
    assert "PbShape::RoundRect;" in face
...
def test_display_runtime_prefers_psram_and_fails_closed_when_unavailable():
    ...
    # 0.0.47: the shared helper carries the PSRAM-first policy and the
    # internal-heap fallback; the render runtime still fails closed.
    helper = (ROOT / "hardware" / "firmware" / "deskbot_task.cpp").read_text(
        encoding="utf-8"
    )
    assert "deskbot_task_create_pinned(" in ensure
    assert "&s_task_stack_uses_psram" in ensure
    assert "MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT" in helper
...
def test_display_governor_limits_spi_work_during_full_duplex_audio():
    ...
    # 0.0.47: 80 MHz default, overridable back to the verified 40 MHz.
    assert "DESKBOT_DISPLAY_SPI_HZ 80000000UL" in panel
    assert "-DDESKBOT_DISPLAY_SPI_HZ=40000000" in panel
