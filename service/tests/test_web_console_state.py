...
def test_device_api_and_console_expose_microphone_acoustic_alarm(
    ...
    # 全局横幅已撤：告警只在首页「Agent 全面日志」内展示，console_2c.js
    # 不再渲染麦克风相关内容。
    assert message not in console
    assert message in home
    assert "mic_no_acoustic_signal" not in console
...
"""
    ...
    # 麦克风异常不再挂全局横幅（告警搬进首页 Agent 全面日志）；
    # 设备在线且无其它异常时横幅全程保持隐藏。
    assert payload["alarm"]["hidden"] is True
    assert "麦克风没有收到声音" not in str(payload["alarm"]["html"])
    assert payload["recovered"]["hidden"] is True
...
def test_control_pages_wait_for_device_terminal_completion():
    ...
    playbooks = (root / "templates" / "app2c" / "playbooks.html").read_text(
        encoding="utf-8"
    )
    # 实验台只保留舵机；摄像头/PB 表情/流水日志模块已删除，场景编排独立成页。
    assert "operation_id:operationId" in lab
    assert lab.count("submitControlOperation(") >= 1
    assert "舵机动作已在设备执行完成" in lab
    for gone in ("PB 表情", "Face catalog", "流水日志", "openCamera"):
        assert gone not in lab
    assert "submitControlOperation(" in playbooks
    assert "operationId" in playbooks
    assert "表演已在设备完成" in playbooks
...
def test_servo_web_controls_submit_logical_steps_atomically():
    ...
    # chunk 结构逻辑随场景编排页独立：编辑器只提升纯预设，
    # __custom__ 等高级形态原样保留，存取往返不丢字段。
    playbooks = (root / "templates" / "app2c" / "playbooks.html").read_text(
        encoding="utf-8"
    )
    assert "if(moves.length) out.moves=moves" in playbooks
    assert "if(anims.length) out.anims=anims" in playbooks
    assert "c.text||c.moves||c.anims" in playbooks
    assert "__custom__" in playbooks
    assert "cleanChunk" not in lab
...
def test_scene_web_adapters_round_trip_canonical_parallel_lanes():
    ...
    root = Path(__file__).parents[1] / "src" / "deskbot_server" / "web"
    playbooks_path = root / "templates" / "app2c" / "playbooks.html"
...
const pbPage = loadComponent({json.dumps(str(playbooks_path))});
const pbCtx = Object.assign({{}}, pbPage.methods, {{computed:null}});
const pbDraft = pbCtx.editable(legacy);
const pbSaved = pbCtx.clean(pbDraft.chunks ? pbDraft : legacy);
const pbChunk = pbSaved.chunks[0];
assert(!("servo" in pbChunk) && !("expr" in pbChunk), "playbooks emitted singular fields");
assert(pbChunk.moves.length === 2 && pbChunk.anims.length === 2, "playbooks lost or duplicated lanes");
assert(pbChunk.moves[0].move === "center" && pbChunk.moves[1].move === "look_left", "playbooks move order changed");
assert(pbChunk.anims[0].anim === "happy" && pbChunk.anims[1].anim === "blink", "playbooks anim order changed");
