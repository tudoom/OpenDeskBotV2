...
def test_resolve_api_key_uses_unified_llm_key(monkeypatch):
    ...
    # 旧变量即使存在也被忽略。
    monkeypatch.setenv("ARK_API_KEY", "ark-stale")
    assert ark_face_svg._resolve_api_key() == "llm-unified"
...
def test_coerce_accepts_single_object_layers_not_only_arrays():
    """回归：模型常把单图元的层写成对象（eye_l:{...}）而非数组（eye_l:[{...}]）。
    早先 _coerce_grouped_model_elements 对非数组直接 continue，导致眼/鼻/嘴整层被丢、
    生成的脸残缺（只剩碰巧写成数组的部位）。单对象必须被当作单元素数组保留。"""
    from deskbot_server.ark_face_svg import _coerce_model_scene
        ...
        "frames": [
            {
                "ms": 220,
                "elements": {
                    "eye_l": {"shape": "ellipse_fill", "x": 86, "y": 96, "rw": 17, "rh": 8},
                    "eye_r": {"shape": "ellipse_fill", "x": 198, "y": 96, "rw": 17, "rh": 8},
                    "nose": {"shape": "circle", "x": 142, "y": 124, "r": 11},
                    "mouth": {"shape": "round_rect_outline", "x": 112, "y": 148, "w": 60, "h": 14, "radius": 7},
                },
            }
        ],
    }
    coerced = _coerce_model_scene(scene)
    el = coerced["frames"][0]["elements"]
    assert len(el["eye_l"]) == 1
    assert len(el["eye_r"]) == 1
    assert len(el["nose"]) == 1
    assert len(el["mouth"]) == 1


def test_model_element_keeps_color_for_carved_mouth():
    """回归：手绘库的弧形嘴 = 白色 round_rect 上叠一个 c=0（黑）的小 round_rect 抠出来；
    早先转换器只保留几何、丢掉颜色，黑色抠嘴变回缺省色、弧形消失。c 整数须原样透传，
    css 颜色（color/fill）须作为 color 保留给归一化校验。"""
    from deskbot_server.ark_face_svg import _model_element_to_primitive

    black = _model_element_to_primitive({"shape": "round_rect", "x": 146, "y": 154, "w": 62, "h": 13, "radius": 6, "c": 0})
    assert black is not None and black["c"] == 0
    pink = _model_element_to_primitive({"shape": "ellipse_fill", "x": 55, "y": 131, "rw": 12, "rh": 6, "fill": "#fa6040"})
    assert pink is not None and pink["color"] == "#fa6040"
    plain = _model_element_to_primitive({"shape": "circle", "x": 142, "y": 124, "r": 11})
    assert plain is not None and "c" not in plain and "color" not in plain


def test_model_element_supports_rotated_rect_and_triangle():
    """坏笑/撇嘴这类"一边翘"的嘴只能用带角度的矩形画；固件、预览、归一化都支持
    rotated_rect_fill(x/y 中心, w/h, angle) 与 triangle_fill(x1..y3)，转换器不得再把它们丢掉。"""
    from deskbot_server.ark_face_svg import _model_element_to_primitive

    rr = _model_element_to_primitive({"shape": "rotated_rect_fill", "x": 150, "y": 158, "w": 56, "h": 12, "angle": -12, "c": 2047})
    assert rr == {"shape": "rotated_rect_fill", "x": 150, "y": 158, "w": 56, "h": 12, "angle": -12.0, "c": 2047}
    tri = _model_element_to_primitive({"shape": "triangle_fill", "x1": 120, "y1": 150, "x2": 180, "y2": 150, "x3": 150, "y3": 170})
    assert tri["shape"] == "triangle_fill" and tri["x3"] == 150 and tri["y3"] == 170
    ln = _model_element_to_primitive({"shape": "line", "x1": 80, "y1": 80, "x2": 120, "y2": 76, "sw": 4})
    assert ln["sw"] == 4
