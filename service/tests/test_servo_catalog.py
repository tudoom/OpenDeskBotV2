...
def test_catalog_rejects_absolute_steps_outside_configured_limits():
    with pytest.raises(ValueError, match=r"absolute step y=135 outside \[78, 110\]"):
        normalize_servo_document(
...
def test_rtc_move_tool_drops_preset_enum_when_catalog_has_no_model_visible_presets(monkeypatch):
    """没有可见预设时 move_head 仍可用（自由组合 steps），但不能生成
    ``"enum": []`` 这种非法 schema，也不能让模型去选一个不存在的预设。"""
    from deskbot_server.rtc_worker_tools import build_rtc_tool_schemas
    ...
    move = next(s for s in build_rtc_tool_schemas() if s.get("name") == "move_head")
    props = move["parameters"]["properties"]
    assert "move" not in props
