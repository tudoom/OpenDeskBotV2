...
def test_merged_emotion_expressions_includes_builtin():
    ...
    assert "sad" in names
    assert "fake_smile" in names
    assert "wry_smile" not in names
    assert len(merged) >= 6
            ...
            "phonemes": [
                {
                    ...
                    "frames": [
                        ...
                        },
                        {
                            "ms": 400,
                            "elements": {
                                "mouth": [{"shape": "rect", "x": 9, "y": 9, "w": 9, "h": 9}]
                            },
                        },
                    ],
                }
...
def test_save_emotions_to_design_file(tmp_path, monkeypatch):
    ...
    builtin_names = {row["name"] for row in builtin_emotion_scenes()}
    assert len(rows) == 1 + len(builtin_names)
    assert rows[0]["name"] == "happy"
    reloaded = load_face_expr_scenes_file(seed_if_missing=False)
    assert reloaded is not None
    assert {r["name"] for r in reloaded} == {"happy", *builtin_names}
    local_path = tmp_path / "local" / "deskbot-face.json"
    on_disk = json.loads(local_path.read_text(encoding="utf-8"))
    assert [row["name"] for row in on_disk["emotions"]] == ["happy"]


def test_resolve_face_design_path_is_pc_local(tmp_path, monkeypatch):
    global_dir = tmp_path / "global"
    global_dir.mkdir()
    ...
    expected = str(tmp_path / "local" / "deskbot-face.json")
    assert resolve_face_design_path() == expected
