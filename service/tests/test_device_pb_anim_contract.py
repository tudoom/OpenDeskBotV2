...
def test_servo_with_scene_carries_scene_assets_for_bitmap_scenes():
    """舵机动作附带的表情若是位图（卡通）场景，operation_payload 必须带 scene_assets，
    运行时按它补发 JPEG；只带 frames 到设备就是 image asset=0 invalid（2026-09-07 全链路排查）。"""
    from pathlib import Path

    source = Path(__file__).resolve().parents[1].joinpath("src/deskbot_server/ws/http_api.py").read_text(encoding="utf-8")
    assert '"scene_assets": scene_assets_durable' in source
    assert 'normalize_scene_assets(operation_payload.get("scene_assets"))' in source
