...
def test_collect_missing_servo_presets(tmp_path, monkeypatch):
    ...
    servo_path.write_text(
        '{"xMin":30,"xMax":150,"yMin":78,"yMax":110,"xReverse":0,"yReverse":0,'
...
def test_legacy_pose_moves_migrate_to_custom_and_save(tmp_path, monkeypatch):
    """早期姿态网格的 pose_x{X}_y{Y} 预设名：读取/保存时就地转坐标动作。"""
    ...
    monkeypatch.setattr(
        "deskbot_server.servo_config_store.servo_preset_catalog",
        lambda **_kw: [{"id": "center", "label": "回中", "steps": [{"ms": 400}]}],
    )
    norm = store.normalize_scene_playbooks(rows)
    converted = store.migrate_legacy_pose_moves(norm)
    assert converted == 1
    moves = norm[0]["chunks"][0]["moves"]
    assert moves[0]["move"] == "__custom__"
    assert moves[0]["x"] == 104 and moves[0]["y"] == 90
    assert moves[0]["xm"] == 0 and moves[0]["ym"] == 0
    assert moves[1]["move"] == "center"
    # 迁移后不再有未知预设，保存不会被硬拒。
    assert store.collect_missing_servo_presets(norm) == []
