...
def test_worker_creation_and_enqueue_failures_are_fail_fast_and_owning():
    ...
    # 0.0.47: PSRAM-preferred stack via the shared helper (static task, PSRAM
    # stack, internal TCB, dynamic fallback) instead of an inline WithCaps pair.
    assert "deskbot_task_create_pinned(" in ensure
    assert "&stack_in_psram" in ensure
