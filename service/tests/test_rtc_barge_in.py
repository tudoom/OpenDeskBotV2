...
def test_ordinary_barge_in_requires_sustained_audio_but_stop_does_not():
    ...
    assert should_confirm_barge_in("你喜欢吃什么", speech_seconds=0.9) is False  # 温和：不满 1 s 不算
    assert should_confirm_barge_in(
...
def test_three_cjk_characters_confirm_an_ordinary_interruption():
    ...
    # characters with ≥1.0 s of audio are a deliberate interruption（2026-09-10 温和方案）.
    assert should_confirm_barge_in("那不对", speech_seconds=1.0) is True
    assert should_confirm_barge_in("太快了", speech_seconds=1.3) is True
    # Still gated on sustained near-end audio, not text length alone.
    assert should_confirm_barge_in("那不对", speech_seconds=0.7) is False
...
def test_english_interruption_behaviour_is_unchanged():
    ...
    # Ordinary English interruptions need the 1.0 s sustained floor.
    assert should_confirm_barge_in("that is wrong", speech_seconds=0.9) is False
...
def test_sdk_min_interruption_words_stays_at_one_word():
    ...
    # 2026-09-10 温和打断：VAD 连续说满 1.0 s 才挂起播放
    assert 'kwargs.setdefault("min_interruption_duration", 1.0)' in source
