...
# ---------------------------------------------------------------------------
# PB bundle atomicity (2026-09-07): a JSON declaration and every binary it
# declares must be contiguous on PB_WIRE.  Concurrent bitmap expression /
# TTS / camera-control senders used to interleave, and the firmware reported
# "BIN fragment overflow" and failed the sequence.
# ---------------------------------------------------------------------------


def _pb_wire_bundles(fake: FakeSerial) -> list[tuple[str, int]]:
    """Return [(req, delivered_binary_bytes)] in wire order.

    Every JSON frame opens a new bundle entry; non-JSON PB_WIRE frames add to
    the most recent one.  A binary that lands before a *later* declaration
    therefore counts for the wrong bundle and the totals mismatch.
    """
    ...
    for frame in _decode_writes(fake):
        if frame.channel != Channel.PB_WIRE:
            continue
        if frame.flags & FrameFlag.JSON:
            assembling += frame.payload
            if frame.flags & FrameFlag.BEGIN_STREAM and not (
                frame.flags & FrameFlag.END_STREAM
            ):
                continue
            if (
                frame.flags & (FrameFlag.BEGIN_STREAM | FrameFlag.END_STREAM)
                and not frame.flags & FrameFlag.END_STREAM
            ):
                continue
            decoded = json.loads(assembling.decode())
            assembling = b""
            bundles.append((str(decoded.get("req")), 0))
        else:
            assert bundles, "binary before any declaration"
            req, total = bundles[-1]
            bundles[-1] = (req, total + len(frame.payload))
    return bundles


def _bundle_wire(req: str, next_bin_len: int) -> str:
    return json.dumps(
        {
            "type": "pb_single",
            "req": req,
...
def test_concurrent_pb_bundles_stay_contiguous_on_pb_wire():
    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        bin_a = b"a" * 4884
        ...
        bin_c = b"c" * 6008
        await asyncio.gather(
            session.send_pb_wire(_bundle_wire("A", len(bin_a)), binaries=[bin_a]),
            session.send_pb_wire(_bundle_wire("B", len(bin_b)), binaries=[bin_b]),
            session.send_pb_wire(_bundle_wire("C", len(bin_c)), binaries=[bin_c]),
        )
        bundles = _pb_wire_bundles(fake)
        expected = {"A": len(bin_a), "B": len(bin_b), "C": len(bin_c)}
        assert sorted(req for req, _ in bundles) == ["A", "B", "C"]
        for req, delivered in bundles:
            assert delivered == expected[req], bundles
        assert session._pending_pb_binary_lengths == []
        assert not session._pb_bundle_lock.locked()
        await session.close()

    asyncio.run(_run())


def test_split_declaration_holds_bundle_until_binaries_are_sent():
    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        bin_a = b"a" * 4000
        ...
        await session.send_pb_wire(_bundle_wire("A", len(bin_a)))
        assert session._pb_bundle_lock.locked()
        other = asyncio.create_task(
            session.send_pb_wire(_bundle_wire("B", len(bin_b)), binaries=[bin_b])
        )
        await asyncio.sleep(0.05)
        assert not other.done()
        assert [req for req, _ in _pb_wire_bundles(fake)] == ["A"]

        await session.send_pb_binary(bin_a)
        await other
        assert _pb_wire_bundles(fake) == [("A", len(bin_a)), ("B", len(bin_b))]
        assert not session._pb_bundle_lock.locked()
        await session.close()
...
def test_stalled_pb_bundle_is_evicted_and_late_binary_is_rejected():
    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        session._pb_bundle_stall_seconds = 0.2
        bin_a = b"a" * 1000
        bin_b = b"b" * 500

        await session.send_pb_wire(_bundle_wire("A", len(bin_a)))
        started = time.monotonic()
        await session.send_pb_wire(_bundle_wire("B", len(bin_b)), binaries=[bin_b])
        assert time.monotonic() - started >= 0.15
        assert _pb_wire_bundles(fake) == [("A", 0), ("B", len(bin_b))]
        assert not session._pb_bundle_lock.locked()

        with pytest.raises(SerialProtocolError):
            await session.send_pb_binary(bin_a)
        assert not session._pb_bundle_lock.locked()
        # A healthy bundle right after the eviction must not wait again.
        started = time.monotonic()
        await session.send_pb_wire(_bundle_wire("C", 10), binaries=[b"c" * 10])
        assert time.monotonic() - started < 0.15
        await session.close()
...
def test_pb_cancel_releases_a_held_bundle():
    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        await session.send_pb_wire(_bundle_wire("A", 1000))
        assert session._pb_bundle_lock.locked()
        await session.send_pb_wire(
            json.dumps({"type": "pb_cancel", "req": "A"}, separators=(",", ":"))
        )
        assert not session._pb_bundle_lock.locked()
        assert session._pending_pb_binary_lengths == []
        started = time.monotonic()
        await session.send_pb_wire(_bundle_wire("B", 5), binaries=[b"bbbbb"])
        assert time.monotonic() - started < 0.15
        await session.close()
...
def test_cancelled_bundle_sender_still_delivers_declared_binaries():
    """A superseded expression / barge-in cancels the caller between the JSON
    declaration and its binaries.  The firmware already waits for those bytes,
    so the session must finish the bundle on its own and release the lock."""

    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        bin_a = b"a" * 9000  # several fragments -> real await points
        ...
        with pytest.raises(asyncio.CancelledError):
            await task
        await _eventually(
            lambda: _pb_wire_bundles(fake) == [("A", len(bin_a))], timeout=2.0
        )
        await _eventually(lambda: not session._pb_bundle_lock.locked())
        assert session._pending_pb_binary_lengths == []
        # The next bundle goes straight through, after A's last fragment.
        await session.send_pb_wire(_bundle_wire("B", 7), binaries=[b"b" * 7])
        assert _pb_wire_bundles(fake) == [("A", len(bin_a)), ("B", 7)]
        await session.close()

    asyncio.run(_run())


def test_camera_once_does_not_hold_pb_bundle_while_audio_is_down():
    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        await session.begin_audio_down_stream(b"opus")
        camera = json.dumps(
            {"type": "pb_single", "req": "cam", "camera_once": True},
            separators=(",", ":"),
        )
        # Skipped during audio down: must return 0 without touching the lock.
        assert await session.send_pb_wire(camera) == 0
        assert not session._pb_bundle_lock.locked()
        started = time.monotonic()
        await session.send_pb_wire(_bundle_wire("A", 5), binaries=[b"aaaaa"])
        assert time.monotonic() - started < 0.15
        await session.close()
...
def test_caller_cancelled_during_declaration_still_completes_bundle():
    """Cancellation while the declaration's last fragment awaits its ACK must
    not leave the firmware with a declaration and no binaries."""

    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        bin_a = b"a" * 3000
        wire = _pb_wire_with_size(4000, next_bin_len=len(bin_a))
        task = asyncio.create_task(session.send_pb_wire(wire, binaries=[bin_a]))
        # Cancel as soon as the first JSON fragment is on the wire.
        await _eventually(
            lambda: any(
                frame.channel == Channel.PB_WIRE and frame.flags & FrameFlag.JSON
                for frame in _decode_writes(fake)
            )
        )
        task.cancel()
        ...
        await _eventually(
            lambda: sum(
                len(f.payload)
                for f in _decode_writes(fake)
                if f.channel == Channel.PB_WIRE and not f.flags & FrameFlag.JSON
            )
            == len(bin_a),
            timeout=2.0,
        )
        ...
        json_bytes = b"".join(
            f.payload
            for f in _decode_writes(fake)
            if f.channel == Channel.PB_WIRE and f.flags & FrameFlag.JSON
        )
        assert json_bytes == wire.encode()
        await session.close()

    asyncio.run(_run())


def test_pb_ack_for_same_req_implies_lost_transport_ack():
    """The firmware answered with pb_ack (frame consumed) but the transport
    frame_ack never reached the host; the session must not time out."""

    async def _run() -> None:
        fake = FakeSerial(auto_frame_ack=False)
        session = await _start_ready_session(fake)
        session._frame_ack_timeout = 0.5
        wire = json.dumps(
            {"type": "pb_single", "req": "r1", "anim": []},
            separators=(",", ":"),
        )
        sending = asyncio.create_task(session.send_pb_wire(wire))
        await _eventually(
        ...
        pb_ack = json.dumps(
            {"type": "pb_ack", "req": "r1", "idx": 0, "phase": "accepted"},
            separators=(",", ":"),
        ).encode()
        fake.inject(
            encode_frame(
        ...
        await asyncio.wait_for(sending, timeout=0.4)
        assert session.implied_frame_acks == 1
        assert session._pending_frame_acks == {}
        await session.close()

    asyncio.run(_run())


def test_hello_and_heartbeat_carry_ack_counters():
    async def _run() -> None:
        fake = FakeSerial()
        session = await _start_ready_session(fake)
        heartbeat = json.dumps(
            {
                ...
                "session_epoch": session.session_epoch,
                "ack_sequence": 1,
                "usb_ack_send_failures": 3,
                "usb_ack_retry_ok": 2,
                "usb_poll_max_gap_ms": 40,
            },
            separators=(",", ":"),
        ).encode()
        fake.inject(
            encode_frame(
                ...
                sequence=78,
                session_epoch=session.session_epoch,
                flags=FrameFlag.JSON | FrameFlag.ACK,
            )
        )
        await _eventually(
            lambda: session.diagnostics(now=time.monotonic()).usb_ack_send_failures == 3
        )
        snapshot = session.diagnostics(now=time.monotonic())
        assert snapshot.usb_ack_retry_ok == 2
        assert "usb_ack_send_failures=3" in session._diagnostic_summary(now=time.monotonic())
        await session.close()
