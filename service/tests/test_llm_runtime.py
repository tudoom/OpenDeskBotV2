...
def test_resolve_system_llm_config_prefers_volcengine_env(monkeypatch):
...
def test_resolve_system_llm_config_defaults_to_deepseek(monkeypatch):
    """2026-09-11 用户决定：什么都没配时出厂默认 DeepSeek deepseek-flash（此前 MiMo mimo-v2.5）。"""
    from deskbot_server import env as dotenv_module
    ...
    assert cfg.model == DEFAULT_LLM_MODEL == "deepseek-flash"
    assert cfg.protocol == "openai"
    assert cfg.api_base == DEEPSEEK_OPENAI_BASE_URL == "https://api.deepseek.com"
    assert cfg.api_key == "deepseek-test-key"
...
def test_missing_key_message_mentions_volcengine_env():
    ...
    assert "ARK_API_KEY" not in str(exc.value)
