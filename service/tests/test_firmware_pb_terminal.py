...
def test_all_modalities_share_absolute_chunk_timeline_without_drift():
    ...
    assert "deskbot_pb_time_reached(now_ms, deadline_ms)" in motor
...
def test_relative_servo_is_resolved_by_actor_and_duration_stays_uint32():
    ...
    assert "resolve_target_safe(cmd.xm, x, cmd.x" in actor
...
def test_pb_motor_rate_limit_extends_deadline_without_final_snap():
    """2026-09-09 梯形速度曲线：步速上限 2°/拍、迟到补追也不超速、到位后等截止时刻，绝不在截止时刻直接跳到目标。"""
    head = _read("head.cpp")
    header = _read("head.h")
    actor = _between(head, "void motor_task", "void drain_motor_queue_nonblocking")
    pb_motion = actor[actor.index("if (moving) {") : actor.index("/* 纯保持段")]
    assert "SERVO_MAX_STEP_DEG_PER_TICK = 2;" in header
    assert "int v_max = SERVO_MAX_STEP_DEG_PER_TICK;" in pb_motion
    assert "step_toward(x, ideal_x, v_max)" in pb_motion
    assert "step_toward(y, ideal_y, v_max)" in pb_motion
    assert "profile_init(px, x_start, x_target, v_max, alloc_ticks)" in pb_motion
    assert "const bool arrived = (x == x_target) && (y == y_target);" in pb_motion
    assert "deskbot_pb_time_reached(now_ms, deadline_ms)" in pb_motion
...
def test_servo_interpolation_is_signed_unified_and_hardware_bounded():
    """线性插值已被梯形速度曲线取代：加速时间固定、给的时间更长就降巡航速度、写入前夹到硬包络。"""
    head = _read("head.cpp")
    helper = _between(head, "void profile_init(", "int profile_position(")
    assert "p.a = v / ta;" in helper
    assert "if (p.d >= v * ta) {" in helper                      # 梯形
    assert "p.t_acc = sqrtf(p.d / p.a);" in helper               # 三角
    assert "if (v_fit < v) {" in helper                          # 时间富余 → 降速而不是提前到达干等
    assert "p.dir = delta > 0 ? 1 : (delta < 0 ? -1 : 0);" in helper  # 有符号方向，不混无符号
    travel = _between(head, "float profile_travel(", "int profile_position(")
    assert "if (n >= t_end) {" in travel and "return p.d;" in travel

    actor = _between(head, "void motor_task", "void drain_motor_queue_nonblocking")
    assert "interpolate_axis_clamped(" not in actor
    ...
    assert "constrain(step_toward(x, ideal_x, v_max), X_MIN_LIMIT, X_MAX_LIMIT)" in actor
    assert "constrain(step_toward(y, ideal_y, v_max), Y_MIN_LIMIT, Y_MAX_LIMIT)" in actor
    assert "const uint32_t start_ms = millis();" in actor
    assert "kPbMotorExecutionGraceMs" in actor
    assert "watchdog_deadline_ms" in actor
...
def test_servo_new_soft_range_does_not_fabricate_the_starting_pose():
    """目标在软限位（留 3° 边缘）内解析；起点永远是最后一次写出的 PWM 角，不按新软限位伪造。"""
    head = _read("head.cpp")
    resolver = _between(head, "int resolve_target_safe(", "}  // namespace")
    assert "SERVO_EDGE_MARGIN_DEG" in resolver and "SERVO_DEADBAND_DEG" in resolver
    assert "当前角本身可能在边缘外" in resolver
    actor = _between(head, "void motor_task", "void drain_motor_queue_nonblocking")
    assert actor.index("int x_target = resolve_target_safe") < actor.index("motor_wait_for_pb_start")
    assert "x = constrain(x, cmd.x_min, cmd.x_max)" not in actor
    assert "y = constrain(y, cmd.y_min, cmd.y_max)" not in actor
    # 曲线从本段起始角出发（x_start 就是上一次写出的 PWM 角），整段固定，逐拍只算理想位置。
    assert "const int x_start = x;" in actor
    assert "profile_init(px, x_start, x_target" in actor
    assert "profile_position(px, x_start, n)" in actor
    assert "profile_position(px, x, n)" not in actor
