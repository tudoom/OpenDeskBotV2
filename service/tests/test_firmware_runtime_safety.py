...
def test_v2_servo_axes_match_production_mechanics_and_safe_travel():
    ...
    # 2026-09-09：转头轴 10/170 收到 20/160（预设最远 30/150，再外面是撞壳区）。
    assert "#define X_MIN_LIMIT 20" in head
    assert "#define X_MAX_LIMIT 160" in head
    # 2026-09-08：70 是顶在外壳上的堵转位（USB 过流断电），收到 78。
    assert "#define Y_MIN_LIMIT 78" in head
...
def test_ack_required_frames_are_confirmed_before_application_dispatch():
    ...
    # 0.0.39: a failed ack write is deferred to the poll retry (bounded by
    # kAckRetryWindowMs) on every link instead of ending the session here.
    assert 'usb_transport_end_session("frame_ack_send_failed")' not in dispatch
    assert "s_pending_wifi_ack_since_ms = millis();" in dispatch
...
def test_camera_capture_failures_reinitialise_driver_with_rate_limit():
    ...
    # 单次 no_frame 抖动很常见：3 次就拆驱动会把抖动放大成永久失明，阈值放宽
    # 到 8 次 / 30s，并且内存不足以重建时绝不拆、驱动已拆时不再徒劳重建。
    assert "kCameraFailureRecoveryThreshold = 8u" in camera
    assert "kCameraRecoveryMinIntervalMs = 30000u" in camera
    assert "kCameraReinitMinLargestBlockBytes = 40u * 1024u" in camera
    # 驱动已拆时的重建门槛必须低于运行期"不拆"门槛：实测 31.7KB 连续块能 init。
    assert "kCameraSetupMinLargestBlockBytes = 24u * 1024u" in camera
    ...
    # 内存门槛：驱动还活着 → 跳过重建（keeping driver）；驱动已拆 → 推迟 setup。
    assert "driver reset skipped" in uploader
    assert "setup deferred" in uploader
    assert uploader.index("camera_internal_largest_block()") < uploader.index("esp_camera_deinit()")
...
def test_usb_transition_parks_radio_without_deinit_reinit():
    """2026-09-04 台架实证的射频坏态根因：USB/WiFi 切换时反复 esp_wifi_deinit→esp_wifi_init
    让 PHY 恢复出坏态（nvs_enable=0，校准不持久化），WiFi 12ms/0% → 100% 丢包且不自愈。
    修法：USB 会话期间只 esp_wifi_disconnect（协议栈常驻），离开时 esp_wifi_connect 重连，
    射频从不经历 deinit/reinit。这里锁死：USB 挂起路径不得再调 esp_wifi_deinit/esp_wifi_stop。"""
    link = _read("wifi_link.cpp")
    park = _between(link, 'if (usb_host_active) {', 'if (s_radio_suspended) {')
    assert "esp_wifi_disconnect();" in park
    assert "esp_wifi_deinit()" not in park
    assert "esp_wifi_stop()" not in park
    assert 'wifi_link_forensic("radio parked (usb host)");' in park
    unpark = _between(link, "if (s_radio_suspended) {", "if (!s_wifi_inited) {")
    # 恢复路径默认不再 init（只在开机没 init 成时兜底），且用 connect 重连
    assert "if (!s_wifi_inited && !lean_wifi_init())" in unpark
    assert "start_sta();" in unpark


def test_camera_control_frames_take_the_media_hint_fast_path():
    """仅带 cam_fps / camera_once 的控制帧不占任何通道，必须和 mic 提示帧一样走快速路径直接应用
    并回 played：进播放队列会在卡通 9s 时间线常驻期间被当"跨请求 append"拒掉（拍照/预览失败），
    改成 replace 又会取消正在播的卡通序列（2026-09-07 真机实测）。"""
    asr = _read("asr_chat_client.cpp")
    fast = _between(asr, "const bool camera_once_early", "const uint32_t chunk_ms = preflight.chunk_ms;")
    assert "has_cam_control_early" in fast
    assert "camera_request_snapshot();" in fast
    assert "camera_set_fps((uint32_t)fps_early);" in fast
    assert "PbSequenceOutcome::kPlayed" in fast
    # 快速路径必须仍然拒绝带显示/舵机/音频/附件的文档，避免绕过队列仲裁
    assert "!pb_doc_has_nonempty_anim(doc) && !pb_doc_has_nonempty_servo(doc)" in fast
    assert "!pb_doc_has_asset_bins(doc)" in fast
    # 快速路径出现在队列仲裁（pbDecideChainHead）之前
    assert asr.index("has_cam_control_early") < asr.index("const PbQueueDecision qd = pbDecideChainHead(chunk_level, chunk_action);")


def test_pb_interleaving_fails_the_sequence_not_the_transport():
    """2026-09-07: a foreign binary inside a fragmented PB JSON used to tear
    down the whole USB session (host saw "frame ACK timeout").  The frames
    are intact; only the PB layer is misaligned, so fail that PB and keep the
    link."""
    asr = _read("asr_chat_client.cpp")
    branch = _between(
        asr,
    ...
    assert 'pbProtocolError("binary interleaved with JSON fragments")' in branch
    assert "resetTransportSession(" not in branch
    drain = _between(
        asr,
        "void AsrChatClient::pbDrainWorkersForNewSequence",
        "if (cancel_terminals) {",
    )
    assert "new sequence while awaiting BIN" in drain


def test_camera_capture_pause_is_logged_with_its_reason():
    source = _read("camera.cpp")
    loop = _between(source, "if (capture_paused()) {", "const bool snapshot_requested =")
    assert "[CAMERA] capture paused" in loop
    assert "speaker_audible=" in loop
    assert "rtc_interruptible=" in loop


def test_usb_frame_ack_failure_is_retried_before_ending_session():
    """2026-09-07: one failed CONTROL_JSON ack write over USB used to end the
    session immediately (host: "frame ACK timeout" right when the camera
    fallback stream started).  Defer and retry inside the host's window."""
    source = _read("usb_transport.cpp")
    poll = _between(
        source,
    ...
    assert "kAckRetryWindowMs" in poll
    assert 'usb_transport_end_session("frame_ack_send_failed")' in poll
    send = _between(source, "bool send_with_epoch(", "bool partial_invalidated = false;")
    assert "if (channel == DESKBOT_USB_CAMERA_JPEG) {\n    budget_ms = 200u;" in send


def test_usb_session_ack_window_matches_firmware_retry():
    manager = (ROOT / "service" / "src" / "deskbot_server" / "infrastructure" / "serial" / "manager.py").read_text(encoding="utf-8")
    usb_ctor = manager.split("session._reuse_port = True")[0].rsplit("DeviceSession(", 1)[1]
    assert "frame_ack_timeout=6.0" in usb_ctor


def test_camera_keeps_sensor_warm_while_idle():
    """0.0.41: idle (interval=0, no snapshot) grabs and discards one frame per
    second so a later one-shot does not pay the OV3660 4–6 s cold start."""
    source = _read("camera.cpp")
    idle = _between(
        source,
    ...
    assert "kCameraKeepWarmIntervalMs" in idle
    assert "esp_camera_fb_get()" in idle
    assert "esp_camera_fb_return(warm)" in idle
    assert "usb_transport_send" not in idle


def test_usb_framed_tx_bypasses_hwcdc_ring_buffer():
    """0.0.46: arduino-esp32 3.x HWCDC guesses host presence from SOF and,
    under load, flips to "unplugged" — its ISR then stops draining, write()
    evicts queued bytes and flush() discards the ring.  Byte accounting on the
    host showed 24-byte headers arriving with their payload+CRC gone.  Framed
    TX therefore feeds the 64-byte hardware IN endpoint directly and the
    session-boundary cleanup only drains, never discards."""
    source = _read("usb_transport.cpp")
    loop = _between(source, "bool write_all(", "void notify_link(bool ready);")
    assert "usb_serial_jtag_ll_txfifo_writable()" in loop
    assert "usb_serial_jtag_ll_write_txfifo(" in loop
    assert "usb_serial_jtag_ll_txfifo_flush()" in loop
    assert "kCdcPacketBytes = 64u" in source
    assert "HWCDCSerial.isConnected()" not in loop
    cleanup = _between(source, "bool flush_stale_cdc_tx()", "void seek_magic_byte(")
    # the only remaining HWCDCSerial.flush() is the pre-mutex bring-up branch
    assert cleanup.count("HWCDCSerial.flush()") == 1
    assert "availableForWrite()" in cleanup


def test_camera_jpeg_is_sent_from_a_private_copy():
    """0.0.44: the framebuffer keeps being written by DMA; sending it in place
    let the payload change between CRC and transmission (host: one payload CRC
    failure per 2-3 frames, frame-aligned)."""
    source = _read("camera.cpp")
    upload = _between(source, "CameraUploadResult try_upload_one_frame(", "void camera_uplink_task(")
    assert "memcpy(private_copy, jpeg_payload, jpeg_len);" in upload
    assert upload.index("esp_camera_fb_return(fb);\n      fb = nullptr;") < upload.index("usb_transport_send_for_epoch(")


def _created_task_names() -> set[str]:
    """Every task name this firmware creates (any creation API)."""
    import re

    names: set[str] = set()
    pattern = re.compile(
        r'(?:xTaskCreatePinnedToCore(?:WithCaps)?|deskbot_task_create_pinned)\s*\(\s*'
        r'\w+\s*,\s*"([^"]+)"'
    )
    for path in FW.glob("*.cpp"):
        names.update(pattern.findall(path.read_text(encoding="utf-8")))
    return names


def test_stack_heap_snapshot_is_cheap_and_forwards_one_warn_line():
    trace = _read("task_trace.cpp")
    header = _read("task_trace.h")
    boot = _read("deskbot_rom.ino")

    # loop() hooks the tick right after the task stall tick.
    loop_body = boot[boot.index("void loop()") :]
    assert loop_body.index("log_task_tick();") < loop_body.index(
        "log_stack_heap_tick();"
    )

    assert "#define DESKBOT_STACK_HEAP_INFO_MS 5000" in header
    assert "#define DESKBOT_STACK_HEAP_WARN_MS 30000" in header

    tick = _between(trace, "void log_stack_heap_tick()", "LogTaskScope::LogTaskScope")
    assert "xTaskGetHandle(" in tick
    assert "uxTaskGetStackHighWaterMark(" in tick
    assert "heap_caps_get_info(" in tick
    assert "MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT" in tick
    assert "MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT" in tick
    # Per-task detail stays on the serial monitor; the PC gets one WARN line.
    assert 'log_info("[STACK]' in tick
    assert 'log_info("[HEAP]' in tick
    assert tick.count("log_warn(") == 1
    assert 'log_warn("[MEM] stack_min=' in tick
    assert "DESKBOT_STACK_HEAP_WARN_MS" in tick
    # No dynamic allocation inside the tick.
    for forbidden in ("malloc(", "new ", "String(", "std::string", "snprintf(", "vTaskList", "uxTaskGetSystemState"):
        assert forbidden not in tick, forbidden

    # The watch table covers every task the firmware creates.
    table = _between(trace, "const StackWatch kStackWatch[]", "};")
    watched = set(__import__("re").findall(r'\{"([^"]+)",', table))
    created = _created_task_names()
    assert created, "no task creation sites found"
    missing = created - watched
    assert not missing, f"tasks missing from kStackWatch: {sorted(missing)}"
    assert "loopTask" in watched


def test_afe_signal_stats_only_computes_rms_on_the_fetch_path():
    source = _read("audio_frontend_esp_sr.cpp")
    signature = _between(source, "SignalStats signal_stats(", "{")
    assert "bool need_rms = false" in signature
    body = _between(source, "SignalStats signal_stats(", "struct SignalWindow")
    assert body.count("sqrt(") == 1
    assert body.index("if (need_rms)") < body.index("sqrt(")
    # Only the AFE fetch path (VAD volume) asks for rms; feed/health paths do not.
    fetch = _between(
        source, "void audio_frontend_fetch_task", "void cleanup_failed_setup"
    )
    assert "/*need_rms=*/true" in fetch
    feed = _between(source, "void audio_frontend_feed_task", "void audio_frontend_fetch_task")
    assert "need_rms" not in feed
    assert source.count("/*need_rms=*/true") == 1


def test_camera_ov3660_sensor_tuning_is_guarded_and_keeps_qvga_policy():
    source = _read("camera.cpp")
    block = _between(source, "if (sensor->id.PID == OV3660_PID) {", "CameraProbeRecord activity;")
    for setter, value in (
        ("set_sharpness", "3"),
        ("set_contrast", "1"),
        ("set_bpc", "1"),
        ...
        assert f"if (sensor->{setter} != nullptr)" in block, setter
        assert f"sensor->{setter}, {value}, false" in block, setter
    # Sensor tuning must not change the runtime frame size / capture policy.
    assert "set_framesize" not in block
    assert "#define DESKBOT_CAMERA_RUNTIME_FRAME_SIZE FRAMESIZE_QVGA" in source


def test_display_spi_clock_is_a_build_flag_with_documented_fallback():
    header = _read("display_panel.h")
    panel = _read("display_panel.cpp")
    guard = _between(header, "#ifndef DESKBOT_DISPLAY_SPI_HZ", "#endif")
    assert "#define DESKBOT_DISPLAY_SPI_HZ 80000000UL" in guard
    assert "-DDESKBOT_DISPLAY_SPI_HZ=40000000" in guard
    assert "setSPISpeed(DESKBOT_DISPLAY_SPI_HZ)" in panel
    assert "(unsigned long)DESKBOT_DISPLAY_SPI_HZ" in panel


def test_psram_task_stacks_are_limited_to_large_flash_free_tasks():
    import re

    # The prebuilt Arduino SDK never applies sdkconfig.defaults, and the boot
    # contract script rejects wiring it in.  The file records the requirement
    # (the prebuilt SDK already satisfies it) for a future hybrid build.
    ini = (ROOT / "hardware" / "platformio.ini").read_text(encoding="utf-8")
    assert "board_build.sdkconfig_defaults" not in ini
    contract = (ROOT / "hardware" / "scripts" / "check_usb_boot_contract.py").read_text(encoding="utf-8")
    assert '"board_build.sdkconfig_defaults":' in contract
    defaults = _read("sdkconfig.defaults")
    assert "CONFIG_SPIRAM=y" in defaults
    assert "CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY=y" in defaults
    assert "check_usb_boot_contract.py" in defaults

    helper = _read("deskbot_task.cpp")
    assert "heap_caps_malloc(stack_bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)" in helper
    assert "sizeof(StaticTask_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT" in helper
    assert "xTaskCreateStaticPinnedToCore(" in helper
    # Fallback keeps the firmware alive without PSRAM and never leaks half an allocation.
    assert "return xTaskCreatePinnedToCore(" in helper
    assert helper.index("heap_caps_free(stack);") < helper.index("return xTaskCreatePinnedToCore(")
    assert "stack_bytes < 1024u" in helper
    assert "flash/NVS" in _read("deskbot_task.h")

    moved = (
        ("mic_cap", "audio_capture.cpp"),
        ("esp_sr_feed", "audio_frontend_esp_sr.cpp"),
        ("esp_sr_fetch", "audio_frontend_esp_sr.cpp"),
        ("audio_play", "audio_player.cpp"),
        ("display_render", "display.cpp"),
    )
    for name, filename in moved:
        source = _read(filename)
        assert re.search(
            r'deskbot_task_create_pinned\(\s*\w+,\s*"%s"' % name, source
        ), name
        assert not re.search(
            r'xTaskCreatePinnedToCore(?:WithCaps)?\(\s*\w+,\s*"%s"' % name, source
        ), name
        # Ratchet: a PSRAM-stack task must never write flash/NVS.
        for forbidden in ("nvs_", "Preferences", "esp_partition_write", "esp_partition_erase", "esp_ota_", "pb_completed_store"):
            assert forbidden not in source, (filename, forbidden)

    # Small tasks and the NVS-writing loopTask stay on internal stacks; the
    # on-demand Opus workers keep their own WithCaps policy (no fallback).
    kept_internal = (
        ("motor", "head.cpp"),
        ("camera_usb", "camera.cpp"),
        ("runtime_guard", "runtime_supervisor.cpp"),
        ("idle_headroom", "task_trace.cpp"),
    )
    for name, filename in kept_internal:
        assert re.search(
            r'xTaskCreatePinnedToCore\(\s*\w+,\s*"%s"' % name, _read(filename)
        ), name
    boot = _read("deskbot_rom.ino")
    assert "SET_LOOP_TASK_STACK_SIZE(24576);" in boot
    assert "deskbot_task_create_pinned" not in boot
    for filename in ("opus_uplink.cpp", "opus_downlink.cpp", "rtc_audio_downlink.cpp"):
        assert "xTaskCreatePinnedToCoreWithCaps(" in _read(filename)
        assert "deskbot_task_create_pinned" not in _read(filename)
