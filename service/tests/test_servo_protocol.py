...
def test_physical_completion_budget_covers_short_full_range_batches():
    steps = [
        {"xm": 0, "ym": 0, "x": 30, "y": 78, "ms": 50}
        for _ in range(32)
    ]
    # 最坏行程 130°（从包络另一端 160 出发）→ 梯形 1°/拍 + 5 拍加减速 = 135 拍 = 2700 ms，×32。
    assert servo_sequence_completion_budget_ms(steps) == 32 * 2700
...
def test_physical_budget_does_not_assume_pose_is_inside_new_soft_bounds():
    ...
    # X may still be at hardware endpoint 20/160 from an older configuration:
    # 70° worst travel -> 70 + 5 accel ticks = 75 ticks * 20 ms = 1500 ms.
    assert servo_sequence_completion_budget_ms([step]) == 1500
...
@pytest.mark.parametrize(
    "mutate, match",
    [
        (lambda doc: doc.update(yMin=60), "yMin must be between 78 and 110"),
        (
