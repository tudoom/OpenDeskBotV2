...
def test_confirmation_is_bound_to_exact_payload_and_replays_once(temp_db, monkeypatch):
    ...
    # 米家写操作已免确认，这里用同样受确认门控的「删除提醒」验证确认绑定。
    def fake_execute_schedule_task_tool(raw):
        provider_calls.append(dict(raw))
        return {"tool": "schedule_task", "ok": True, "result": "deleted"}

    monkeypatch.setattr(
        runner, "execute_schedule_task_tool", fake_execute_schedule_task_tool
    )
    command = {
        "tool": "schedule_task",
        "action": "delete",
    ...
    assert pending[0]["confirmation_required"] is True
    ...
    assert provider_calls[0]["task_id"] == "task-a"
...
def test_provider_success_then_ledger_failure_is_never_replayed(
    ...
    # 米家写操作已免确认，改用同样受确认门控的「删除提醒」。
    def fake_execute_schedule_task_tool(raw):
        provider_calls.append(dict(raw))
        return {"tool": "schedule_task", "ok": True, "result": "provider-accepted"}
