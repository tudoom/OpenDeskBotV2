...
function Install-MilocoMiotWheel {
    ...
    Write-Host "  Miloco SDK:   $($wheel.Name)"
}
...
try {
    ...
    # The Miloco SDK (miloco-miot) carries native libraries for linux and darwin only — there
    ...
    if (Test-Path -LiteralPath $miotLibs) {
        Write-Host "Removing the Miloco SDK's linux/darwin native libraries..."
    ...
    Copy-Item `
        ...
        -Force
    # 内部分发预置凭证（可选）：client\seed.env 存在时打进 seed\.env。
    # 该文件在 .gitignore 中，只存在于构建机；launcher 首装缺 .env 才落地。
    $seedEnvSource = Join-Path $clientRoot "seed.env"
    if (Test-Path -LiteralPath $seedEnvSource -PathType Leaf) {
        $seedEnvDest = Join-Path $seedRoot ".env"
        Copy-Item `
            -LiteralPath $seedEnvSource `
            -Destination $seedEnvDest `
            -Force
        ...
        if ($seedRaw.Length -gt 0 -and -not $seedRaw.EndsWith("`n")) {
            [System.IO.File]::AppendAllText($seedEnvDest, [Environment]::NewLine)
        }
        $seedLines = @(Get-Content -LiteralPath $seedEnvDest -ErrorAction SilentlyContinue)
        foreach ($kv in $bundledEnvDefaults) {
            $key = $kv.Substring(0, $kv.IndexOf("="))
            $pattern = "^\s*" + [regex]::Escape($key) + "="
            $present = $false
            foreach ($line in $seedLines) {
                if ($line -match $pattern) { $present = $true; break }
            }
            if (-not $present) {
                Add-Content -LiteralPath $seedEnvDest -Value $kv
                Write-Host "  Seeded default: $key"
            }
        }
    ...
    # 构建目录可能不是有效的 git 仓库（曾是已删除仓库的 worktree），
    # 版本戳取不到 commit 时如实标 unknown，而不是让整次构建炸掉。
    $commit = "unknown"
    try {
        $raw = & git -C $serviceRoot rev-parse HEAD 2>$null
        if ($LASTEXITCODE -eq 0 -and $raw) { $commit = "$raw".Trim() }
    } catch {}
    $manifest = [ordered]@{
